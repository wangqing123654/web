 ...
 * @version 1.0
 */
public class OPBFeeListPrintTool extends TJDOTool {
	/**
	 * 实例
	 */
	public static OPBFeeListPrintTool instanceObject;
	 ...
	 */
	public static OPBFeeListPrintTool getInstance() {
		if (instanceObject == null)
			instanceObject = new OPBFeeListPrintTool();
	...
	public TParm getSumPrintData(String caseNo, String optUser, String admType) {
		...
		String sql="";
		if (admType.equals("O")) {
			regSql = "SELECT B.PAT_NAME,B.MR_NO,B.BIRTH_DATE,MAX(C.BILL_DATE) BILL_DATE,D.CONTRACTOR_DESC AS INSURANCE_COMPANY,A.ADM_TYPE "
					+ "FROM REG_PATADM A,SYS_PATINFO B,BIL_OPB_RECP C,MEM_CONTRACTOR D"
					+ " WHERE A.CASE_NO=C.CASE_NO AND A.MR_NO=B.MR_NO AND A.INSURE_INFO=D.CONTRACTOR_CODE(+) "
					+ " AND A.CASE_NO='"
					+ caseNo
					+ "'"
					+ " GROUP BY B.PAT_NAME,B.MR_NO,B.BIRTH_DATE,D.CONTRACTOR_DESC,A.ADM_TYPE ";
			sql = "SELECT ORDER_CODE,ORDER_DESC,SPECIFICATION,DOSAGE_QTY, DOSAGE_UNIT,OWN_AMT, AR_AMT,"
					...
					+ " CASE WHEN (E.PRESCRIPTION_FLG IS NULL OR E.PRESCRIPTION_FLG='N') AND D.CAT1_TYPE='PHA' THEN '★' ELSE '' END||D.TRADE_ENG_DESC AS ORDER_DESC,'' SPECIFICATION,"
					...
					+ " SYS_FEE D,SYS_UNIT B,PHA_BASE E"
					+ " WHERE A.ORDER_CODE = D.ORDER_CODE AND  A.DOSAGE_UNIT = B.UNIT_CODE(+) "
					+ " AND a.REXP_CODE = C.ID AND C.GROUP_ID = 'SYS_CHARGE' AND D.ORDER_CODE=E.ORDER_CODE(+) "
					+ " AND A.CASE_NO = '"
					+ caseNo
					+ "' AND A.RECEIPT_NO IS NOT NULL AND A.OWN_AMT<>0 "
					+ " GROUP BY A.ORDER_CODE,D.TRADE_ENG_DESC,A.DOSAGE_UNIT,A.REXP_CODE, A.HEXP_CODE,C.ENG_DESC,B.UNIT_ENG_DESC,E.PRESCRIPTION_FLG,D.CAT1_TYPE "
					+ " UNION ALL SELECT  A.ORDER_CODE,"
					+ " CASE WHEN (E.PRESCRIPTION_FLG IS NULL OR E.PRESCRIPTION_FLG='N') AND D.CAT1_TYPE='PHA' THEN '★' ELSE '' END||A.ORDER_DESC,'' SPECIFICATION, 0 AS DOSAGE_QTY,"
					...
					+ caseNo
					+ "' AND A.RECEIPT_NO IS NOT NULL AND A.OWN_AMT<>0"
					+ " GROUP BY A.ORDER_CODE, A.ORDER_DESC, A.DOSAGE_UNIT, A.REXP_CODE, A.HEXP_CODE,C.CHN_DESC,B.UNIT_CHN_DESC,E.PRESCRIPTION_FLG,D.CAT1_TYPE ) ORDER BY  REXP_CODE,ORDER_CODE,DOSAGE_QTY DESC";
		}else{
			regSql = "SELECT B.PAT_NAME,B.MR_NO,B.BIRTH_DATE,MAX(C.CHARGE_DATE) BILL_DATE,D.CONTRACTOR_DESC AS INSURANCE_COMPANY "
				+ "FROM ADM_INP A,SYS_PATINFO B,BIL_IBS_RECPM C,MEM_CONTRACTOR D"
				+ " WHERE A.CASE_NO=C.CASE_NO(+) AND A.MR_NO=B.MR_NO AND A.INSURE_INFO=D.CONTRACTOR_CODE(+) "
				+ " AND A.CASE_NO='"
				+ caseNo
				+ "'"
				+ " GROUP BY B.PAT_NAME,B.MR_NO,B.BIRTH_DATE,D.CONTRACTOR_DESC ";
			sql = "SELECT ORDER_CODE,ORDER_DESC,SPECIFICATION,DOSAGE_QTY,DOSAGE_UNIT,OWN_AMT,AR_AMT,"
				+ "REXP_CODE,HEXP_CODE,CHN_DESC,UNIT_CHN_DESC "
				+ " FROM ( SELECT   A.ORDER_CODE,"
				+ " CASE WHEN (E.PRESCRIPTION_FLG IS NULL OR E.PRESCRIPTION_FLG='N') AND D.CAT1_TYPE='PHA' THEN '★' ELSE '' END||D.TRADE_ENG_DESC AS ORDER_DESC,'' SPECIFICATION, SUM(A.DOSAGE_QTY) DOSAGE_QTY,"
				...
				+ " GROUP BY A.ORDER_CODE,D.TRADE_ENG_DESC,A.DOSAGE_UNIT,A.REXP_CODE, A.HEXP_CODE,C.ENG_DESC,B.UNIT_ENG_DESC,E.PRESCRIPTION_FLG,D.CAT1_TYPE "
				+ " UNION ALL SELECT  A.ORDER_CODE,"
				+ " CASE WHEN (E.PRESCRIPTION_FLG IS NULL OR E.PRESCRIPTION_FLG='N') AND D.CAT1_TYPE='PHA' THEN '★' ELSE '' END||A.ORDER_CHN_DESC ORDER_DESC,'' SPECIFICATION, 0 AS DOSAGE_QTY,"
				+ " A.DOSAGE_UNIT ,0 OWN_AMT,0 AR_AMT,"
				+ " A.REXP_CODE, A.HEXP_CODE,C.CHN_DESC,B.UNIT_CHN_DESC "
				+ " FROM IBS_ORDD A,SYS_DICTIONARY C,"
				+ " SYS_FEE D,SYS_UNIT B,PHA_BASE E"
				...
				+ caseNo
				+ "' AND A.BILL_FLG='Y' AND A.OWN_AMT<>0"
				+ " GROUP BY A.ORDER_CODE,A.ORDER_CHN_DESC, A.DOSAGE_UNIT,A.REXP_CODE, A.HEXP_CODE,C.CHN_DESC,B.UNIT_CHN_DESC,E.PRESCRIPTION_FLG,D.CAT1_TYPE ) ORDER BY  REXP_CODE,ORDER_CODE,DOSAGE_QTY DESC";
		}
		...
		print.setData("INSURANCE_COMPANY", "TEXT", regParm.getValue("INSURANCE_COMPANY", 0));
