 ...
 * @version 1.0
 */
public class IndBaseManageMTool 
    extends TJDOTool {
    /**
     * 实例
     */
    public static IndBaseManageMTool instanceObject;
     ...
     */
    public static IndBaseManageMTool getInstance() {
        if (instanceObject == null)
            instanceObject = new IndBaseManageMTool();
     ...
     * 构造器
     */
    public IndBaseManageMTool() {
        setModuleName("spc\\INDBaseManageMModule.x");
        onInit();
    }
    ...
    public TParm onQuery(TParm parm) {
        TParm result = this.query("queryBaseManageM", parm);
    ...
    public TParm onInsert(TParm parm, TConnection conn) {
        TParm result = this.update("createNewBaseManageM", parm, conn);
        if (result.getErrCode() < 0) {
            err("ERR:" + result.getErrCode() + result.getErrText()
                + result.getErrName());
            return result;
    ...
    public TParm onUpdate(TParm parm, TConnection conn) {
        TParm result = this.update("updateBaseManageM", parm, conn);
        if (result.getErrCode() < 0) {
            err("ERR:" + result.getErrCode() + result.getErrText()
                + result.getErrName());
            return result;
    ...
    public TParm onDelete(TParm parm, TConnection conn) {
        TParm result = this.update("deleteBaseManageM", parm, conn);
        if (result.getErrCode() < 0) {
            err("ERR:" + result.getErrCode() + result.getErrText()
                + result.getErrName());
            return result;
