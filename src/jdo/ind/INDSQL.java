...
public class INDSQL {
	...
	public static String getINDSysParm() {
		String sql =  "SELECT FIXEDAMOUNT_FLG,REUPRICE_FLG,DISCHECK_FLG,GPRICE_FLG,UNIT_TYPE,"
		...
		+ "PHA_PRICE_FLG,TOXIC_LENGTH,TOXBOX_LENGTH,IS_SEPARATE_REQ,DEAF_MON," 
		+ "STAT_START,STAT_END FROM IND_SYSPARM";
	...
	public static String getMaterialloc() {
		return "SELECT ORG_CODE , MATERIAL_LOC_CODE , MATERIAL_CHN_DESC , MATERIAL_ENG_DESC ,PY1 ,"
				...
				+ " OPT_TERM FROM IND_MATERIALLOC ORDER BY SEQ ";
	}
	 ...
	 * 
	 */
	public static String getINDPurPlanD(String org_code , String plan_no , String plan_month){
		String sql =  "WITH T AS (SELECT A.TRANDATE,A.ORG_CODE,A.ORDER_CODE,SUM (A.STOCK_QTY) AS LAST_QTY "
           			+ " FROM IND_DDSTOCK A "
           			+	" WHERE A.TRANDATE = '"+plan_month+"' || (SELECT SUBSTR(TRIM(TO_CHAR(STAT_END,'00')),-2) FROM JAVAHIS.IND_SYSPARM) "
           			+ " GROUP BY A.TRANDATE, A.ORG_CODE, A.ORDER_CODE) "
           			+ " SELECT A.ORG_CODE,A.PLAN_NO,A.ORDER_CODE,T.LAST_QTY "
           			+ " FROM IND_PURPLAND A LEFT JOIN T ON A.ORG_CODE = T.ORG_CODE AND A.ORDER_CODE = T.ORDER_CODE "
           			+ "WHERE A.ORG_CODE = '"+ org_code +"' AND A.PLAN_NO = '"+plan_no+"' ORDER BY A.SEQ";
		return sql;
	}
	...
	public static String getIndStockQty(String org_code, String order_code,
		...
		return "SELECT A.BATCH_SEQ,A.BATCH_NO,A.VALID_DATE,"
				...
				+ " AND B.ORDER_CODE = C.ORDER_CODE " + " AND A.ORG_CODE = '"
				+ org_code + "' AND A.ORDER_CODE = '" + order_code
				+ "' AND A.ACTIVE_FLG='Y'  AND SYSDATE < A.VALID_DATE  " + " ORDER BY A.VALID_DATE " + sort
				+ " , A.BATCH_SEQ";
	}
	 ...
	 * @return String
	 */
	public static String getIndStockQty(String org_code, String order_code,int batchseq,
			String sort) {
		return "SELECT A.BATCH_SEQ,A.BATCH_NO,A.VALID_DATE,"
				+ " A.STOCK_QTY  AS QTY,"
				+ " C.OWN_PRICE AS RETAIL_PRICE, C.OWN_PRICE2, C.OWN_PRICE3, "
	...
	public static String getIndStockAsc(String org_code, String order_code,
			String sort) {
		return "SELECT A.BATCH_SEQ,A.BATCH_NO,A.VALID_DATE,"
				+ " A.STOCK_QTY  AS QTY,"
				+ " C.OWN_PRICE AS RETAIL_PRICE, C.OWN_PRICE2, C.OWN_PRICE3, "
				...
				+ " AND B.ORDER_CODE = C.ORDER_CODE " + " AND A.ORG_CODE = '"
				+ org_code + "' " + " AND A.ORDER_CODE = '" + order_code + "' "
				+ " AND A.ACTIVE_FLG='Y' AND A.STOCK_QTY > 0 AND  SYSDATE < A.VALID_DATE "
				+ " ORDER BY A.VALID_DATE ASC , A.BATCH_SEQ ASC ";
	}
	...
	public static String getIndPlanAdvice(String org_code, String sup_code,
		...
		return "SELECT 'N' AS SELECT_FLG, A.ORDER_CODE, B.ORDER_DESC, B.SPECIFICATION, "
				...
				+ "A.PLAN_NO FROM IND_PURPLAND A, PHA_BASE B,IND_AGENT C "
				+ "WHERE A.ORDER_CODE = B.ORDER_CODE AND A.ORDER_CODE=C.ORDER_CODE AND A.ORG_CODE = '"
				+ org_code
				+ "' AND A.SUP_CODE = '"
				+ sup_code
	...
	public static String getIndQtyProfitLoss(String org_code, String frozen_date) {
		...
		return "SELECT C.DEPT_CHN_DESC, CASE "
				...
				// add by wangbin 2014-11-27 外部需求 #486 增加盘盈亏合计金额 START
				+ ",(A.ACTUAL_CHECK_QTY - A.STOCK_QTY) * A.VERIFYIN_PRICE AS PROFIT_LOSS_PRICE"
				// add by wangbin 2014-11-27 外部需求 #486 增加盘盈亏合计金额 END
				+ " FROM IND_QTYCHECK A, IND_STOCK B, SYS_DEPT C, SYS_FEE D,  "
				+ " PHA_TRANSUNIT E, SYS_UNIT F, SYS_UNIT G "
				// luhai modify 2012-05-09 begin 盘点表打印时indstock 使用左连 begin
				+ " WHERE A.ORG_CODE = B.ORG_CODE(+) AND A.ORDER_CODE = B.ORDER_CODE(+) "
