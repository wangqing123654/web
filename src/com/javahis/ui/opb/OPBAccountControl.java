...
//		}
    ...
    public void onPrint() {
       ...
       // System.out.println("tableParm"+tableParm);
        ...
        String seq = "";
        int rowCount = 0;
//        System.out.println("+++++tableParm tableParm is ::"+tableParm);
        for (int i = 0; i < count; i++) {
        ...
        if ("N".equals(getValue("TOGEDER_FLG"))) {
            for (int i = 0; i < rowCount; i++) {
                ...
                printData.setData("accountDate1","日结日期:"+endParm.getData("ACCOUNT_DATE",i).toString().substring(0,19));//日结日期 add by huangjw 20150202
//                System.out.println("ppppppooooooprintData printData is ::"+printData);
                //======yanjing 添加会员卡套餐日结数据 start
               TParm packParm = this.onPrintPackData(singleParm);//yanjing 20140505收入
               TParm packTrParm = this.onTrPrintPackData(singleParm);//yanjing 20140505退款
               TParm packAllParm = this.onAllPrintPackData(singleParm);//yanjing 20140505总计
...
//                System.out.println("====+++==== printData printData is ::"+printData);
...
//                System.out.println("printData printData printData is：：："+printData);
//                this.openPrintWindow(
...
//        returnParm.setData("PRINTNO", printNo.getd);
//        System.out.println("*******returnParm returnParm returnParm is ::"+returnParm);
...
//            printParm);
        ...
        if (sql == "" || sql.equals(""))
            return null;
//        System.out.println("ttttttsql sql sql is :::"+sql);
...
//======20140616 yanjing 添加抗菌药物和非抗菌药物费用总计  start
            chargeParm.setData("CHARGE1AND2",
            		chargeParm.getDouble("CHARGE02") +
                    chargeParm.getDouble("CHARGE01") );
//======20140616 yanjing 添加抗菌药物和非抗菌药物费用总计  end
...
//            if(parm.getDouble("REDUCE_AMT", i)>0){
//            	chargeParm.setData("REDUCE_AMT",//减免金额
//                        -chargeParm.getDouble("REDUCE_AMT")-
//                         parm.getDouble("REDUCE_AMT", i));
//            }else{
...
//                System.out.println("payaramt========"+payParm.getDouble("AR_AMT"));
                ...
                payParm.setData("PAY_MEDICAL_CARD",
                                ...
                                parm.getDouble("PAY_TYPE11", i));//医疗卡
...
//                                            parm.getDouble("PAY_MEDICAL_CARD", i)));
                		(payParmBack.getDouble("PAY_MEDICAL_CARD") +
                                parm.getDouble("PAY_MEDICAL_CARD", i)+parm.getDouble("PAY_TYPE11", i)));
...
//        System.out.println("输出礼品卡数据printParm ：： "+printParm);
     ...
     * @return TParm
     */
    private TParm getRePrintCancelTableDate(String recpType,String cashierCode,String printDate) {
        DecimalFormat df = new DecimalFormat("##########0.00");
...
//            "    AND ACCOUNT_SEQ IN (" +accountSeq + ") "+
            ...
            "    AND CANCEL_DATE IS NOT NULL AND ACCOUNT_SEQ IS NULL  ";
        selMrNo += " AND LENGTH (INV_NO) < 12";//add by wanglong 20121112 过滤掉12位的建行机器的票据号
//        System.out.println("===+++====selMrNo selMrNo selMrNo is ::"+selMrNo);
     ...
     * @return TParm
     */
    private TParm getRePrintReturnTableDate(String recpType,String cashierCode,String printDate) {
        DecimalFormat df = new DecimalFormat("##########0.00");
        TParm parmData = new TParm();
        String selMrNo =
            ...
            " AND B.RECP_TYPE = 'OPB'" +
            "    AND B.RECP_TYPE = '"+recpType+"' " +
            "    AND B.CASHIER_CODE = '"+cashierCode+"' " +
            "    AND B.PRINT_DATE<TO_DATE('"+printDate+"','YYYYMMDDHH24MISS') " +
            "    AND (B.ACCOUNT_FLG='N' OR B.ACCOUNT_FLG IS NULL) " +
            "    AND CANCEL_DATE IS NOT NULL AND B.ACCOUNT_SEQ IS NULL  "+
            " AND B.CANCEL_FLG = '1'" +
     ...
     * @return TParm
     */
    private TParm getReChangeTableDate(String recpType,String cashierCode,String printDate) {
        DecimalFormat df = new DecimalFormat("##########0.00");
        TParm parmData = new TParm();
        String selMrNo =
            " SELECT INV_NO,AR_AMT FROM BIL_INVRCP " +
	...
	public void toBalacnce(String accountSeq) {
		String sql = " UPDATE BIL_OPB_RECP "
				...
				+ " WHERE PAY_MEDICAL_CARD + PAY_INS_CARD +PAY_OTHER1 +PAY_OTHER2<> AR_AMT "
				+ " AND PAY_INS_CARD = 0 " 
				+ " AND ADM_TYPE IN ('O','E')" 
				+ " AND ACCOUNT_SEQ IN (" + accountSeq + ")";
	...
	private TParm getOPBparm(String account_seq) {
		String sql = " SELECT   '收费' RECP_TYPE, A.INV_NO, B.USER_NAME PRINT_USER " +
						...
						" AND A.RECP_TYPE = 'OPB'";
//		sql += " AND LENGTH (A.INV_NO) < 12";//add by wanglong 20121112 过滤掉12位的建行机器的票据号     delete by huangtt 20141217
	 ...
	 * @return
	 */
	private TParm getReOPBparm(String recpType,String cashierCode,String printDate) {
		String sql = " SELECT   '收费' RECP_TYPE, A.INV_NO, B.USER_NAME PRINT_USER " +
						" FROM BIL_INVRCP A , SYS_OPERATOR B " +
...
//		String accountDate = inParm.getValue("ACCOUNT_DATE",0).toString().substring(0, 19)
//        .replace("-", "").replace("/", "").replace(" ", "").replace(":", "");
		...
		//delete by huangtt 20141216 start
//		//根据accountSeq查询receipt_no
//		String receiptSql = "SELECT RECEIPT_NO FROM BIL_OPB_RECP WHERE ACCOUNT_SEQ IN ("+accountSeq+")";
//		receipteResult = new TParm(TJDODBTool.getInstance().select(receiptSql));
//		String receipteNo = "'";
//		for(int j = 0;j<receipteResult.getCount("RECEIPT_NO");j++){
//			if(j!= receipteResult.getCount("RECEIPT_NO") -1){
//				receipteNo +=  receipteResult.getValue("RECEIPT_NO",j)+"','";
//			}else if (j == receipteResult.getCount("RECEIPT_NO") -1){
//				receipteNo +=  receipteResult.getValue("RECEIPT_NO",j)+"'";
////				receipteResult.getData("ACCOUNT_SEQ", j);
//			}
//		}
...
//				" FROM BIL_OPB_RECP B,(SELECT RECEIPT_NO FROM OPD_ORDER WHERE MEM_PACKAGE_ID IS NOT NULL AND RECEIPT_NO IN ("+receipteNo+") GROUP BY RECEIPT_NO ) A  " +
//				"WHERE A.RECEIPT_NO = B.RECEIPT_NO  AND B.TOT_AMT >=0 " +
//				"AND B.ACCOUNT_SEQ IN ("+accountSeq+")";
		//delete by huangtt 20141216 end
		...
		"SUM(B.CHARGE24) AS CHARGE24,SUM(B.CHARGE25) AS CHARGE25,SUM(B.CHARGE26) AS CHARGE26,SUM(B.CHARGE27) AS CHARGE27,SUM(B.CHARGE28) AS CHARGE28," +
		"SUM(B.CHARGE29) AS CHARGE29,SUM(B.CHARGE30) AS CHARGE30, " +
		"SUM(B.PAY_TYPE09) AS PAY_TYPE09,SUM(B.PAY_TYPE09) AS PAY_TYPE10 " +// add by kangy-20160628------添加微信pay_type09、支付宝收入pay_type10---" +
		" FROM BIL_OPB_RECP B " +
		" WHERE B.TOT_AMT >=0 AND B.MEM_PACK_FLG ='Y' " +
		" AND B.ACCOUNT_SEQ IN ("+accountSeq+")";
		
//				"AND B.BILL_DATE < TO_DATE('"+accountDate+"','YYYYMMDDHH24MISS')";//
//		System.out.println("套餐已结转数据打印，总金额orderSqlorderSql is：："+orderSql);
	 ...
	 * huangtt 20141223
	 */
	public TParm onPrintPackData(String admType,String cashierCode,String accountTime){
		TParm result = new TParm ();
		String orderSql = "SELECT SUM(B.TOT_AMT) AS AR_AMT,SUM(B.CHARGE01) AS CHARGE01,SUM(B.CHARGE02) AS CHARGE02,SUM(B.CHARGE03) AS CHARGE03," +
		"SUM(B.CHARGE04) AS CHARGE04,SUM(B.CHARGE05) AS CHARGE05,SUM(B.CHARGE06) AS CHARGE06,SUM(B.CHARGE07) AS CHARGE07,SUM(B.CHARGE08) AS CHARGE08," +
		"SUM(B.CHARGE09) AS CHARGE09,SUM(B.CHARGE10) AS CHARGE10,SUM(B.CHARGE11) AS CHARGE11,SUM(B.CHARGE12) AS CHARGE12,SUM(B.CHARGE13) AS CHARGE13," +
...
//        .replace("-", "").replace("/", "").replace(" ", "").replace(":", "");
		...
		//delete by huangtt 20141215 start
//		//根据accountSeq查询receipt_no
//		String receiptSql = "SELECT RECEIPT_NO FROM BIL_OPB_RECP WHERE ACCOUNT_SEQ IN ("+accountSeq+")";
//		receipteResult = new TParm(TJDODBTool.getInstance().select(receiptSql));
//		String receipteNo = "'";
...
//			}
//		}
////		System.out.println("==============accountDate accountDate is ::"+accountDate);
//		String orderSql = "SELECT SUM(B.TOT_AMT) AS AR_AMT,SUM(B.CHARGE01) AS CHARGE01,SUM(B.CHARGE02) AS CHARGE02,SUM(B.CHARGE03) AS CHARGE03," +
//				"SUM(B.CHARGE04) AS CHARGE04,SUM(B.CHARGE05) AS CHARGE05,SUM(B.CHARGE06) AS CHARGE06,SUM(B.CHARGE07) AS CHARGE07,SUM(B.CHARGE08) AS CHARGE08," +
//				"SUM(B.CHARGE09) AS CHARGE09,SUM(B.CHARGE10) AS CHARGE10,SUM(B.CHARGE11) AS CHARGE11,SUM(B.CHARGE12) AS CHARGE12,SUM(B.CHARGE13) AS CHARGE13," +
//				"SUM(B.CHARGE14) AS CHARGE14,SUM(B.CHARGE15) AS CHARGE15,SUM(B.CHARGE16) AS CHARGE16,SUM(B.CHARGE17) AS CHARGE17,SUM(B.CHARGE18) AS CHARGE18," +
...
//				"WHERE A.RECEIPT_NO = B.RECEIPT_NO  AND B.TOT_AMT < 0 " +
//				"AND B.ACCOUNT_SEQ IN ("+accountSeq+")";
		//delete by huangtt 20141215 end
...
//				"AND B.BILL_DATE < TO_DATE('"+accountDate+"','YYYYMMDDHH24MISS')";//
//		System.out.println("=====退费=========orderSql orderSql is ::"+orderSql);
        result = new TParm(TJDODBTool.getInstance().select(orderSql));
		return result;
	}
	 ...
	 * huangtt 20141223
	 */
	public TParm onTrPrintPackData(String admType,String cashierCode,String accountTime){
		TParm result = new TParm (); 	
		String orderSql = "SELECT ABS(SUM(B.TOT_AMT)) AS AR_AMT,SUM(B.CHARGE01) AS CHARGE01,SUM(B.CHARGE02) AS CHARGE02,SUM(B.CHARGE03) AS CHARGE03," +
		"SUM(B.CHARGE04) AS CHARGE04,SUM(B.CHARGE05) AS CHARGE05,SUM(B.CHARGE06) AS CHARGE06,SUM(B.CHARGE07) AS CHARGE07,SUM(B.CHARGE08) AS CHARGE08," +
		"SUM(B.CHARGE09) AS CHARGE09,SUM(B.CHARGE10) AS CHARGE10,SUM(B.CHARGE11) AS CHARGE11,SUM(B.CHARGE12) AS CHARGE12,SUM(B.CHARGE13) AS CHARGE13," +
...
//		System.out.println("全部==============orderSql orderSql is ::"+orderSql);
	 ...
	 * huangtt 20141223
	 */
	public TParm onAllPrintPackData(String admType,String cashierCode,String accountTime){
		TParm result = new TParm ();		
		String orderSql = "SELECT SUM(B.TOT_AMT) AS AR_AMT,SUM(B.CHARGE01) AS CHARGE01,SUM(B.CHARGE02) AS CHARGE02,SUM(B.CHARGE03) AS CHARGE03," +
		"SUM(B.CHARGE04) AS CHARGE04,SUM(B.CHARGE05) AS CHARGE05,SUM(B.CHARGE06) AS CHARGE06,SUM(B.CHARGE07) AS CHARGE07,SUM(B.CHARGE08) AS CHARGE08," +
		"SUM(B.CHARGE09) AS CHARGE09,SUM(B.CHARGE10) AS CHARGE10,SUM(B.CHARGE11) AS CHARGE11,SUM(B.CHARGE12) AS CHARGE12,SUM(B.CHARGE13) AS CHARGE13," +
		...
		"SUM(B.CHARGE19) AS CHARGE19,SUM(B.CHARGE20) AS CHARGE20,SUM(B.CHARGE21) AS CHARGE21,SUM(B.CHARGE22) AS CHARGE22,SUM(B.CHARGE23) AS CHARGE23," +
		"SUM(B.CHARGE24) AS CHARGE24,SUM(B.CHARGE25) AS CHARGE25,SUM(B.CHARGE26) AS CHARGE26,SUM(B.CHARGE27) AS CHARGE27,SUM(B.CHARGE28) AS CHARGE28," +
		"SUM(B.CHARGE29) AS CHARGE29,SUM(B.CHARGE30) AS CHARGE30,SUM(B.PAY_TYPE09) AS PAY_TYPE09,SUM(B.PAY_TYPE10) AS PAY_TYPE10 " +
		" FROM BIL_OPB_RECP B" +
		" WHERE B.MEM_PACK_FLG ='Y' " +
		" AND  ADM_TYPE= '"+admType+"' " +
		" AND CASHIER_CODE='"+cashierCode+"'" +
...
//		}
////		System.out.println("==============accountDate accountDate is ::"+accountDate);
//		String orderSql = "SELECT * FROM BIL_OPB_RECP B,(SELECT RECEIPT_NO FROM OPD_ORDER WHERE MEM_PACKAGE_ID IS NOT NULL AND RECEIPT_NO IN ("+receipteNo+") GROUP BY RECEIPT_NO ) A  " +
//				"WHERE A.RECEIPT_NO = B.RECEIPT_NO  AND B.TOT_AMT < 0  " +
//				"AND B.ACCOUNT_SEQ IN ("+accountSeq+")";
////				"AND B.BILL_DATE < TO_DATE('"+accountDate+"','YYYYMMDDHH24MISS')";//
		//delete by huangtt 20141216 end
		
		String orderSql = "SELECT * FROM BIL_OPB_RECP B  " +
...
//		System.out.println("退票==============orderSql orderSql is ::"+orderSql);
	 ...
	 * huangtt 20141223
	 */
	public TParm onTrPrintPackQty(String admType,String cashierCode,String accountTime){
		TParm result = new TParm ();
		String orderSql = "SELECT * FROM BIL_OPB_RECP B  " +
		"WHERE B.MEM_PACK_FLG ='Y'  AND B.TOT_AMT < 0  " +
		" AND  ADM_TYPE= '"+admType+"' " +
...
//		System.out.println("退票==============orderSql orderSql is ::"+orderSql);
	...
	public void onPrintReview(){
		DecimalFormat df = new DecimalFormat("0.00");
		 TParm returnParm = new TParm();
		 returnParm.setData("PRINTUSERS", getValueString("ACCOUNT_USER"));
		getAccountTime();//====================得到日结时间点  yanjing 20140618
        String sql = BILSQL.getReceiptParm(admType,getValueString("ACCOUNT_USER"),accountTime);//查询出需要日结的数据
//        System.out.println("预览界面的sql is：："+sql);
        TParm rePrintParm = new TParm(TJDODBTool.getInstance().select(sql));
        if (rePrintParm.getErrCode() < 0) {
            //System.out.println("收据查找错误 " + receiptParm.getErrText());
            return ;
        }
        TParm endReceiptParm = dealPrintParm(rePrintParm);
        String arAmtWord = com.javahis.util.StringUtil.getInstance().numberToWord(endReceiptParm.getParm("PAYTOT").getDouble("AR_AMT"));
      returnParm.setData("AR_AMT_W", arAmtWord);
      returnParm.setData("RECEIPT", endReceiptParm.getData());
      returnParm.setData("RECEIPT", endReceiptParm.getData());
      ...
      dispenseDate = dealDate(dispenseDate);
      String sqlBil = 
      "SELECT PRINT_NO,BILL_DATE,ACCOUNT_SEQ FROM BIL_OPB_RECP WHERE ADM_TYPE LIKE '%"+admType+"%' AND CASHIER_CODE = '"+
      getValueString("ACCOUNT_USER")+"'  ORDER BY BILL_DATE DESC";
//      System.out.println("3211111sqlBil sqlBil sqlBil is ::"+sqlBil);
      TParm bilParm = new TParm(TJDODBTool.getInstance().select(sqlBil));
      String stardate = bilParm.getData("BILL_DATE", 1).toString();
      String enddate = bilParm.getData("BILL_DATE", 0).toString();
      stardate = stardate.substring(0, 19);
      enddate = enddate.substring(0, 19);
      returnParm.setData("ACCOUNT_DATE", stardate+" 至 "+enddate);
//      returnParm.setData("PRINT_NO","000000000000000");
      String printNosql = BILSQL.getRePrintNo("OPB",admType,getValueString("ACCOUNT_USER"),accountTime);
      TParm printNo = new TParm(TJDODBTool.getInstance().select(printNosql));
      if (printNo.getErrCode() < 0) {
         // System.out.println("打印票号查找错误 " + printNo.getErrText());
          return ;
      ...
      returnParm.setData("PRINTNO", printNo.getData());
//      returnParm.setData("PRINTUSERS", printUserS);
//      returnParm.setData("PRINT_NO", bilParm.getData("PRINT_NO", 0))
//      System.out.println("票号显示为returnParm returnParm：："+returnParm);
      String recp_no = "";
      TParm recParm = getReOPBparm(recpType,getValueString("ACCOUNT_USER"),accountTime);
      for (int i = 0; i < recParm.getCount("PRINT_USER"); i++) {
      	recp_no += recParm.getValue("INV_NOS",i) + ";";
		}
      returnParm.setData("PRINTNOS", recp_no);
      returnParm.setData("PRINTUSERS", recParm.getValue("PRINT_USER",0));
      //============zhangp 20120218 modify end
      String dispenseDateE = StringTool.getString((Timestamp)this.getValue("ACCOUNT_DATEE"),"yyyyMMdd")+this.getValueString("E_TIME").replace(":", "");
      //=================pangben modify 20110414 stop
     //  System.out.println("dispenseDateE"+dispenseDateE);
        ...
        apDate = dealDate(apDate);
        returnParm.setData("PRINT_TIME", apDate);
        TParm printCancleDate = this.getRePrintCancelTableDate(recpType,getValueString("ACCOUNT_USER"),accountTime );
        TParm printReturnDate = this.getRePrintReturnTableDate(recpType,getValueString("ACCOUNT_USER"),accountTime);
        TParm printChangeDate = this.getReChangeTableDate(recpType,getValueString("ACCOUNT_USER"),accountTime);
		returnParm.setData("cancelTable", printCancleDate.getData());
		returnParm.setData("returnFeeTable", printReturnDate.getData());
		returnParm.setData("changeTable", printChangeDate.getData());
		String regionSql = "SELECT REGION_CHN_ABN FROM SYS_REGION WHERE REGION_CODE = 'H01'";
		 TParm regionParm = new TParm(TJDODBTool.getInstance().select(regionSql));
        String region = regionParm.getData("REGION_CHN_ABN", 0).toString();
//        String region = "JJJ";
        
        //add by huangtt 20141223 start-------套餐已结转－－－－－
        TParm packParm = this.onPrintPackData(admType,getValueString("ACCOUNT_USER"),accountTime);//yanjing 20140505收入
        TParm packTrParm = this.onTrPrintPackData(admType,getValueString("ACCOUNT_USER"),accountTime);//yanjing 20140505退款
        TParm packAllParm = this.onAllPrintPackData(admType,getValueString("ACCOUNT_USER"),accountTime);//yanjing 20140505总计
        TParm packQty = this.onTrPrintPackQty(admType,getValueString("ACCOUNT_USER"),accountTime);//退费票据张数
//        System.out.println("+++++packAllParm packAllParm is ::"+packAllParm);
//        System.out.println("+++++packAllParm.getCount() packAllParm.getCount() is ::"+packAllParm.getCount());
        if(packAllParm.getCount()>0){//总计存在值，整理打印数据
     	   if(packQty.getCount()>0){//有退费
     		  returnParm.setData("TR_QTY",packQty.getCount());//退费张数
     	   }else{
     		  returnParm.setData("TR_QTY",0);//退费张数
     	   }
     	returnParm.setData("AR_AMT1", df.format(packParm.getDouble("AR_AMT",0)));//套餐总收入
     	returnParm.setData("TAR_AMT1", df.format(packTrParm.getDouble("AR_AMT",0)));//套餐总退费
     	returnParm.setData("AR_AMT_TOT1", df.format(packAllParm.getDouble("AR_AMT",0)));//收入与退费之和
     	returnParm.setData("PCHARGE01", df.format(packAllParm.getDouble("CHARGE01",0)));
     	returnParm.setData("PCHARGE02", df.format(packAllParm.getDouble("CHARGE02",0)));
     	returnParm.setData("PCHARGE1AND2", df.format(packAllParm.getDouble("CHARGE02",0)+packAllParm.getDouble("CHARGE01",0)));
     	returnParm.setData("PCHARGE03", df.format(packAllParm.getDouble("CHARGE03",0)));
     	returnParm.setData("PCHARGE04", df.format(packAllParm.getDouble("CHARGE04",0)));
     	returnParm.setData("PCHARGE05", df.format(packAllParm.getDouble("CHARGE05",0)));
     	returnParm.setData("PCHARGE06", df.format(packAllParm.getDouble("CHARGE06",0)));
     	returnParm.setData("PCHARGE07", df.format(packAllParm.getDouble("CHARGE07",0)));
     	returnParm.setData("PCHARGE08", df.format(packAllParm.getDouble("CHARGE08",0)));
     	returnParm.setData("PCHARGE09", df.format(packAllParm.getDouble("CHARGE09",0)));
     	returnParm.setData("PCHARGE10", df.format(packAllParm.getDouble("CHARGE10",0)));
     	returnParm.setData("PCHARGE11", df.format(packAllParm.getDouble("CHARGE11",0)));
     	returnParm.setData("PCHARGE12", df.format(packAllParm.getDouble("CHARGE12",0)));
     	returnParm.setData("PCHARGE13", df.format(packAllParm.getDouble("CHARGE13",0)));
     	returnParm.setData("PCHARGE14", df.format(packAllParm.getDouble("CHARGE14",0)));
     	returnParm.setData("PCHARGE15", df.format(packAllParm.getDouble("CHARGE15",0)));
     	returnParm.setData("PCHARGE16", df.format(packAllParm.getDouble("CHARGE16",0)));
     	returnParm.setData("PCHARGE17", df.format(packAllParm.getDouble("CHARGE17",0)));
     	returnParm.setData("PCHARGE18", df.format(packAllParm.getDouble("CHARGE18",0)));
     	returnParm.setData("PCHARGE19", df.format(packAllParm.getDouble("CHARGE19",0)));
     	returnParm.setData("PCHARGE20", df.format(packAllParm.getDouble("CHARGE20",0)));
     	returnParm.setData("PCHARGE21", df.format(packAllParm.getDouble("CHARGE21",0)));
     	returnParm.setData("PCHARGE22", df.format(packAllParm.getDouble("CHARGE22",0)));
     	returnParm.setData("PCHARGE23", df.format(packAllParm.getDouble("CHARGE23",0)));   
        //add by kangy 20160627  start----------添加微信、支付宝收入－－－－－－－－－－ 
     	returnParm.setData("PPAY_TYPE09", df.format(packAllParm.getDouble("PAY_TYPE09",0)));
     	returnParm.setData("PPAY_TYPE10", df.format(packAllParm.getDouble("PAY_TYPE10",0)));
        //add by kangy 20160627  end----------添加微信、支付宝收入－－－－－－－－－－ 

        }
        
        TParm receiptParm = returnParm.getParm("RECEIPT");
        TParm chargeParm = receiptParm.getParm("CHARGE");
        //收费项目
        chargeParm.setData("CHARGE01", chargeParm.getDouble("CHARGE01") - returnParm.getDouble("PCHARGE01"));
        chargeParm.setData("CHARGE02", chargeParm.getDouble("CHARGE02") - returnParm.getDouble("PCHARGE02"));
        chargeParm.setData("CHARGE1AND2", chargeParm.getDouble("CHARGE1AND2") - returnParm.getDouble("PCHARGE1AND2") );
        chargeParm.setData("CHARGE03", chargeParm.getDouble("CHARGE03") - returnParm.getDouble("PCHARGE03"));
        chargeParm.setData("CHARGE04", chargeParm.getDouble("CHARGE04") - returnParm.getDouble("PCHARGE04"));
        chargeParm.setData("CHARGE05", chargeParm.getDouble("CHARGE05") - returnParm.getDouble("PCHARGE05"));
        chargeParm.setData("CHARGE06", chargeParm.getDouble("CHARGE06") - returnParm.getDouble("PCHARGE06"));
        chargeParm.setData("CHARGE07", chargeParm.getDouble("CHARGE07") - returnParm.getDouble("PCHARGE07"));
        chargeParm.setData("CHARGE08", chargeParm.getDouble("CHARGE08") - returnParm.getDouble("PCHARGE08"));
        chargeParm.setData("CHARGE09", chargeParm.getDouble("CHARGE09") - returnParm.getDouble("PCHARGE09"));
        chargeParm.setData("CHARGE10", chargeParm.getDouble("CHARGE10") - returnParm.getDouble("PCHARGE10"));
        chargeParm.setData("CHARGE11", chargeParm.getDouble("CHARGE11") - returnParm.getDouble("PCHARGE11"));
        chargeParm.setData("CHARGE12", chargeParm.getDouble("CHARGE12") - returnParm.getDouble("PCHARGE12"));
        chargeParm.setData("CHARGE13", chargeParm.getDouble("CHARGE13") - returnParm.getDouble("PCHARGE13"));
        chargeParm.setData("CHARGE14", chargeParm.getDouble("CHARGE14") - returnParm.getDouble("PCHARGE14"));
        chargeParm.setData("CHARGE15", chargeParm.getDouble("CHARGE15") - returnParm.getDouble("PCHARGE15"));
        chargeParm.setData("CHARGE16", chargeParm.getDouble("CHARGE16") - returnParm.getDouble("PCHARGE16"));
        chargeParm.setData("CHARGE17", chargeParm.getDouble("CHARGE17") - returnParm.getDouble("PCHARGE17"));
        chargeParm.setData("CHARGE18", chargeParm.getDouble("CHARGE18") - returnParm.getDouble("PCHARGE18"));
        chargeParm.setData("CHARGE19", chargeParm.getDouble("CHARGE19") - returnParm.getDouble("PCHARGE19"));
        chargeParm.setData("CHARGE20", chargeParm.getDouble("CHARGE20") - returnParm.getDouble("PCHARGE20"));
        chargeParm.setData("CHARGE21", chargeParm.getDouble("CHARGE21") - returnParm.getDouble("PCHARGE21"));
        chargeParm.setData("CHARGE22", chargeParm.getDouble("CHARGE22") - returnParm.getDouble("PCHARGE22"));
        chargeParm.setData("CHARGE23", chargeParm.getDouble("CHARGE23") - returnParm.getDouble("PCHARGE23"));
        //add by kangy 20160627  start----------添加微信、支付宝收入－－－－－－－－－－ 
        chargeParm.setData("PAY_TYPE09", chargeParm.getDouble("PAY_TYPE09") - returnParm.getDouble("PPAY_TYPE09"));
        chargeParm.setData("PAY_TYPE10",chargeParm.getDouble("PAY_TYPE10") - returnParm.getDouble("PPAY_TYPE10"));
        //add by kangy 20160627  end----------添加微信、支付宝收入－－－－－－－－－－ 
        receiptParm.setData("CHARGE", chargeParm.getData());
        returnParm.setData("RECEIPT", receiptParm.getData());
        
        //add by huangtt 20141223 end-------套餐已结转－－－－－
        
        returnParm.setData("TITLE", "TEXT",
                           (this.getValue("REGION_CODE") != null &&
                            !this.getValue("REGION_CODE").equals("") ? region :
                            "所有医院") + "门急诊收费日结报表");
        System.out.println("传入参数的parm显示为returnParm returnParm：："+returnParm);
        this.openPrintWindow(
                "%ROOT%\\config\\prt\\opb\\OPBReceiptPrint.jhw", returnParm); 
	}
	/**
     * 导出Excel表格
     */
    public void onExport() {
        ...
        ExportExcelUtil.getInstance().exportExcel(table, "门诊日结报表");
    }
}
