	...
	public void onAll(){
		...
		for (int i = 0; i < parm.getCount("FLG"); i++) {
			if(!parm.getBoolean("FLG", i)){
				re = new TParm();
				re.setData("CASE_NO",parm.getValue("CASE_NO", i)) ;
				re.setData("ORDER_CODE", parm.getValue("ORDER_CODE", i)) ;
				mainTable.setItem(i, "FLG", "Y") ;
				TParm result = TIOM_AppServer.executeAction("action.med.MedNodifyAction", "onUpdateStat", re); 
				if(result.getErrCode()<0){
					if(isDedug){  
						System.out.println(" come in class: MEDNodifyControl ，method ：onAll");
						System.out.println("错误内容："+result);
					}
					mainTable.setItem(i, "FLG", "N") ;
				}
			}
		}
	}
