 ...
 * @version 1.0
 */
public class MemPackageRecpImpl implements IRecpType {
	...
	public TParm onQuery(String mrNo, String patName, String recpStDate,
		...
		// TODO Auto-generated method stub
		String sql =
			" SELECT DISTINCT A.*, B.PAT_NAME, 'N' AS FLG" +
			" FROM MEM_PACKAGE_TRADE_M A, SYS_PATINFO B, MEM_PAT_PACKAGE_SECTION C" +
			" WHERE     A.MR_NO = B.MR_NO" +
			" AND A.REST_FLAG = 'N'" +
			" AND A.TAX_FLG = 'N'" +
			" AND A.TRADE_NO = C.TRADE_NO" +
			" AND A.OPT_DATE BETWEEN TO_DATE ('" + recpStDate + "', 'YYYYMMDDHH24MISS')" +
...
//			" AND C.USED_FLG = 0 ";
	...
	public TParm onSave(TParm recpParm, String taxUser)
    	...
    	for (int i = 0; i < recpParm.getCount("FLG"); i++) {
    		sql = 
    			" UPDATE MEM_PACKAGE_TRADE_M SET TAX_FLG = 'Y', TAX_DATE = SYSDATE, TAX_USER = '" + taxUser + "' " +
	...
	public TParm onQueryCompeleted(String mrNo, String patName,
		...
		String sql =
			...
			" AND A.TAX_FLG = 'Y'" +
			" AND A.TRADE_NO = C.TRADE_NO" +
			" AND A.TAX_DATE BETWEEN TO_DATE ('" + taxStDate + "', 'YYYYMMDDHH24MISS')" +
	...
	public TParm onCancle(TParm recpParm) throws Exception {
    	...
    	for (int i = 0; i < recpParm.getCount("FLG"); i++) {
    		sql = 
    			" UPDATE MEM_PACKAGE_TRADE_M SET TAX_FLG = 'N', TAX_DATE = '', TAX_USER = '' " +
    			" WHERE TRADE_NO = '" + recpParm.getValue("TRADE_NO", i) + "'";
    		parm.addData("SQL", sql);
		}
		return parm;
	...
	public TParm onSaveInvNo(TParm recpParm) throws Exception {
    	...
    	for (int i = 0; i < recpParm.getCount(); i++) {
    		sql = 
    			" UPDATE MEM_PACKAGE_TRADE_M SET TAX_INV_NO = '" + recpParm.getValue("TAX_INV_NO", i) + "' " +
    			" WHERE TRADE_NO = '" + recpParm.getValue("TRADE_NO", i) + "'";
    		parm.addData("SQL", sql);
		}
		return parm;
