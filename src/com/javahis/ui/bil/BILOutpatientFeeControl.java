 ...
 * @version 1.0
 */
public class BILOutpatientFeeControl extends TControl{
    ...
    public void onQuery(){
        ...
        if(yzjm.isSelected()){
    		...
    		"就诊科室,100;医嘱代码,80;医嘱名称,200;减免金额,80,double,########0.00;减免原因,160;" +
      	...
      	table.setColumnHorizontalAlignmentData(send);
        	sql="SELECT DISTINCT ROW_NUMBER() OVER(ORDER BY A.CASE_NO,B.DATA_CODE) AS TID, A.CASE_NO,A.MR_NO,C.PAT_NAME,CASE WHEN C.SEX_CODE = '1' THEN '男' WHEN C.SEX_CODE = '2'" +
			...
			" FROM BIL_REDUCEM A, BIL_REDUCED B,SYS_PATINFO C,REG_PATADM D,SYS_CTZ E,SYS_DEPT F " +
			where+
			" AND A.MR_NO=D.MR_NO AND A.MR_NO=C.MR_NO" +
			" AND A.REDUCE_NO=B.REDUCE_NO" +
			" AND E.CTZ_CODE=D.CTZ1_CODE AND A.MR_NO=D.MR_NO " +
			"AND D.REALDEPT_CODE=F.DEPT_CODE " +
			" AND A.REDUCE_AMT>0 AND A.ADM_TYPE='O' AND D.ADM_TYPE='O'" +
			...
			" AND TO_DATE ('"+end+"','YYYYMMDDHH24MISS')" +
					" AND B.REDUCE_TYPE='2' AND B.DATA_TYPE='2'" +	
			" ORDER BY A.CASE_NO,B.DATA_CODE,A.OPT_DATE"; 
	...
	table.setParmValue(tableParm);
    	...
    	if(qtjm.isSelected()){
    		...
    		double[] sum=new double[20];
    		 head="序号,30;就诊号,100;病案号,80;姓名,60;性别,40;就诊身份,120;减免日期,150;" +
     		"就诊科室,100;减免金额,80,double,########0.00;抗菌费,80,double,########0.00;非抗菌费,80,double,########0.00;中成药费,80,double,########0.00;" +
     		"中草药费,80,double,########0.00;常规检查,80,double,########0.00;治疗费,80,double,########0.00;放射费,80,double,########0.00;手术费,80,double,########0.00;输血费,80,double,########0.00;检验费,80,double,########0.00;" +
     		"挂号费,80,double,########0.00;诊疗费,80,double,########0.00;床位费,80,double,########0.00;CT费,80,double,########0.00;B超费,80,double,########0.00;核磁费,80,double,########0.00;材料费,80,double,########0.00;输氧费,80,double,########0.00;" +
     		"麻醉费,80,double,########0.00;其他费,80,double,########0.00;减免原因,180;项目类别,80;申请人,80,REDUCE_USER;经办人,80,OPT_USER;备注,80";
              ...
              mapParm="TID;CASE_NO;MR_NO;PAT_NAME;SEX_CODE;CTZ_DESC;OPT_DATE;DEPT_CHN_DESC;" +
             		...
             		"CHARGE17;CHARGE18;CHARGE19;CHARGE20;REDUCE_NOTE;"+
     		...
     		"27,right;28,right;29,left;30,left;31,left;32,left;33,left";
         	 table.setHeader(head);
          	table.setParmMap(mapParm);
          	table.setColumnHorizontalAlignmentData(send);
              sql="SELECT ROW_NUMBER() OVER(ORDER BY OPT_DATE) AS TID,A.*  FROM(" +
              		...
              		"G.*,A.REDUCE_NOTE,'其他减免' REDUCE_TYPE,A.REDUCE_USER,A.OPT_USER,NULL AS LOCATION " +
              		" FROM BIL_REDUCEM A, BIL_REDUCED B, SYS_PATINFO C, REG_PATADM D, SYS_CTZ E, SYS_DEPT F," +
              		...
              		" IN ('114.1' CHARGE01,'114.2' CHARGE02,'115' CHARGE03," +
              		"'116' CHARGE04,'107' CHARGE05,'109' CHARGE06,'103' CHARGE07,'112' CHARGE08,'111' CHARGE09,'108' CHARGE10,'101' CHARGE11," +
              		"'102' CHARGE12,'118' CHARGE13,'104' CHARGE14,'105' CHARGE15,'106' CHARGE16,'113' CHARGE17,'110' CHARGE18," +
              		"'117' CHARGE19,'119' CHARGE20)))G " +where+
              		" AND A.MR_NO=D.MR_NO AND A.MR_NO=C.MR_NO" +
              		" AND E.CTZ_CODE=D.CTZ1_CODE AND A.MR_NO=D.MR_NO" +
              		" AND D.REALDEPT_CODE=F.DEPT_CODE" +
              		" AND A.REDUCE_NO=G.REDUCE_NO AND A.REDUCE_AMT>0 " +
              		"AND A.ADM_TYPE='O' AND D.ADM_TYPE='O' AND A.CASE_NO=D.CASE_NO" +
              		"  AND A.RESET_REDUCE_NO IS  NULL " +
              		" AND A.OPT_DATE BETWEEN TO_DATE ('"+start+"','YYYYMMDDHH24MISS')" +
              		" AND TO_DATE ('"+end+"','YYYYMMDDHH24MISS') " +
              		"ORDER BY A.OPT_DATE) A";
              System.out.println(sql);
    		TParm parm = new TParm(TJDODBTool.getInstance().select(sql));
    		 if (parm == null || parm.getCount("CASE_NO") <= 0) {
    	         this.messageBox("没有查询到数据");
	...
	public void onExcel() {
		...
		if(yzjm.isSelected()){
		ExportExcelUtil.getInstance().exportExcel(table, "门诊医嘱减免表");
		}
		if(qtjm.isSelected()){
			ExportExcelUtil.getInstance().exportExcel(table, "门诊其他减免表");
		}
	}
	  public void onMrno(){
