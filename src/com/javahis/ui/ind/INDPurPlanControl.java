...
public class INDPurPlanControl
    ...
    public void onExcel(){
    	...
    	ExportExcelUtil.getInstance().exportExcel(table_D, this.getValueString("PLAN_DESC"));
    }
    ...
    public void onPlan() {
        ...
        if (table_d.getRowCount() > 0) {
            ...
            // 表头数据
            date.setData("TITLE", "TEXT", this.getValue("PLAN_DESC").toString());
...
//                System.out.println("PLAN:"+INDSQL.getOrderInfoByCode(order_code, sup_code, "PLAN"));
                if (inparm == null || inparm.getErrCode() < 0) {
                    this.messageBox("药品信息有误");
                    return; 
                }   
//                System.out.println("inparm:"+inparm);  
                //messageBox(""+inparm.getValue("ORDER_DESC", 0));
                parm.addData("ORDER_DESC", inparm.getValue("ORDER_DESC", 0));
                parm.addData("SPECIFICATION",
                             inparm.getValue("SPECIFICATION", 0));
                parm.addData("PURCH_UNIT", inparm.getValue("UNIT_CHN_DESC", 0));
                parm.addData("LASTPUR_QTY" , table_d.getItemDouble(i, "LASTPUR_QTY"));
                parm.addData("LASTCON_QTY", table_d.getItemDouble(i, "LASTCON_QTY"));
                parm.addData("STOCK_QTY", table_d.getItemDouble(i, "STOCK_QTY"));
                parm.addData("PLAN_QTY",
                             table_d.getItemDouble(i, "CHECK_QTY"));
                parm.addData("STOCK_PRICE", table_d.getItemDouble(i, "STOCK_PRICE"));
//                parm.addData("PRUCH_PRICE",
//                             inparm.getDouble("CONTRACT_PRICE", 0));
                parm.addData("PLAN_SUM",
...
//                                              4));
                parm.addData("SUP_CODE",
                             inparm.getValue("SUP_ABS_DESC", 0));
                parm.addData("MAN_CODE",
                             inparm.getValue("MAN_CHN_DESC", 0));
                 
                total += StringTool.round(inparm.getDouble(
                        "CONTRACT_PRICE",
                        0) * table_d.getItemDouble(i, "PLAN_QTY"), 2);
            ...
            //汇总显示行
            parm.addData("ORDER_DESC", null);
            parm.addData("SPECIFICATION", null);
            parm.addData("PURCH_UNIT", null);
            parm.addData("LASTPUR_QTY" , null);
            parm.addData("LASTCON_QTY", null);
            parm.addData("STOCK_QTY", null);
            parm.addData("PLAN_QTY", null);
            parm.addData("STOCK_PRICE", "汇总金额");
            parm.addData("PLAN_SUM", total);
            parm.addData("SUP_CODE", null);
            parm.addData("MAN_CODE", null);
            ...
            parm.addData("SYSTEM", "COLUMNS", "PURCH_UNIT");
            parm.addData("SYSTEM", "COLUMNS", "LASTPUR_QTY");
            parm.addData("SYSTEM", "COLUMNS", "LASTCON_QTY");
            parm.addData("SYSTEM", "COLUMNS", "STOCK_QTY");
            parm.addData("SYSTEM", "COLUMNS", "PLAN_QTY");
            parm.addData("SYSTEM", "COLUMNS", "STOCK_PRICE");
            parm.addData("SYSTEM", "COLUMNS", "PLAN_SUM");
            parm.addData("SYSTEM", "COLUMNS", "SUP_CODE");
            parm.addData("SYSTEM", "COLUMNS", "MAN_CODE");
            date.setData("TABLE", parm.getData());
            
            //统计信息
            date.setData("TOTAL", "TEXT","汇总金额：" + total);
            //表尾数据
            date.setData("OPT", "TEXT", "制表人: " + Operator.getName());
            // 调用打印方法
            this.openPrintWindow("%ROOT%\\config\\prt\\spc\\SPCMedProcurementPlan.jhw",
                                 date);
        }
        else {
            this.messageBox("没有打印数据");
...
//            date.setData("OPT", "TEXT", "制表人: " + Operator.getName());
//            // 调用打印方法
//            //%ROOT%\\config\\prt\\spc\\Purorder.jhw
//            //%ROOT%\\config\\prt\\IND\\PurPlan.jhw
//            this.openPrintWindow("%ROOT%\\config\\prt\\spc\\SpcPurPlan.jhw",
//                                 date);
//        }  
//        else {
//            this.messageBox("没有打印数据");
...
//    }
    ...
    public void onTableMClicked() {
        ...
        if (row != -1) {
          ...
          //20160621 by guangl 修改细表的sql语句，增加上期库存量计算
            final String planmonth = String.valueOf(Integer.parseInt(plan_month.replace("/", ""))-1);
            TDS tds = new TDS()
            {
            	public Object getOtherColumnValue(TParm parm, int row, String column){
            		String org_code = parm.getValue("ORG_CODE" , row);
            		String plan_no = parm.getValue("PLAN_NO" , row);
            		String order_code = parm.getValue("ORDER_CODE" , row);
            		String sql = INDSQL.getINDPurPlanD(org_code, plan_no, planmonth);
            		String sql2 = "SELECT ORDER_CODE ,ORDER_DESC ,SPECIFICATION, MAN_CHN_DESC, GOODS_DESC FROM PHA_BASE "
            			+ "WHERE ORDER_CODE = '" +order_code +  "'";
            		TParm result = new TParm(TJDODBTool.getInstance().select(sql));
            		TParm result2 = new TParm(TJDODBTool.getInstance().select(sql2));
            		if ("LAST_QTY".equals(column)) {
            			return parm.getDouble("STOCK_QTY",row)+parm.getDouble("LASTCON_QTY",row)-parm.getDouble("LASTPUR_QTY",row);
            		} else if("ORDER_DESC".equals(column)){
            			return result2.getValue("ORDER_DESC" , 0);
            		}else if("SPECIFICATION".equals(column)){
            			return result2.getValue("SPECIFICATION" , 0);
            		}else if("MAN_CODE".equals(column)){
            			return result2.getValue("MAN_CHN_DESC" , 0);
            		}else if("PLAN_SUM".equals(column)){
            			Double price =  parm.getDouble("STOCK_PRICE",row);
            			Double qty = parm.getDouble("CHECK_QTY",row);
            			return price*qty;
            		}
					return "";
            	}
            ...
            String tempSql = INDSQL.getINDPurPlandD(org_code, plan_no);
            tds.setSQL(tempSql);
            tds.retrieve();
            if (tds.rowCount() == 0) {
                seq = 1;
...
//            tableNode.setTable(table_D);
//            tableNode.setOldValue(new Double(0));
//            tableNode.setRow(0);
//            tableNode.setColumn(table_D.getColumnIndex("STOCK_PRICE"));
//            tableNode.setValue(tds.getItemData(0, table_D.getColumnIndex("STOCK_PRICE")));
//            
//            this.onTableDChangeValue(tableNode);
//            
        }
    }
...
//            node.getTable().setItem(row, node.getTable().getColumnIndex("PLAN_SUM"), StringTool.round(price * qty, 2));
//            
        ...
        if (result != null) {
            ...
            for (int i = 0; i < addParm.getCount("ORDER_CODE"); i++) {
                ...
                table.setItem(row, "PURCH_UNIT", addParm.getData("PURCH_UNIT",
                    i));
//                String planmonth = getValueString("PLAN_MONTH").substring(0,7).replace("-", "");
//                String sql =
//                	"SELECT A.TRANDATE,A.ORG_CODE,A.ORDER_CODE,SUM (A.STOCK_QTY) AS LAST_QTY  FROM IND_DDSTOCK A  " +
//                	"WHERE A.TRANDATE = '"+planmonth+"' || (SELECT STAT_DAY FROM JAVAHIS.IND_SYSPARM)  " +
//                			"AND A.ORG_CODE = '" +getValue("ORG_CODE") + "'" + " AND A.ORDER_CODE = '" + addParm.getData("ORDER_CODE",i) + "' " +
//                			"GROUP BY A.TRANDATE, A.ORG_CODE, A.ORDER_CODE";
//                TParm temp = new TParm(TJDODBTool.getInstance().select(sql));
//                System.out.println(table.setItem(row,"LAST_QTY",new Double(1.000)));
//                System.out.println("last qty" + table.getItemDouble(row, "LAST_QTY"));
                table.setItem(row, "LAST_QTY", addParm.getDouble("STOCK_QTY",i) + addParm.getDouble("SELL_QTY",i) - addParm.getDouble("BUY_QTY",i));
...
//                table.setItem(row, "PLAN_QTY", addParm.getDouble("PLAN_QTY", i));
//                table.setItem(row, "PUR_QTY", addParm.getDouble("PLAN_QTY", i));
//                table.setItem(row, "CHECK_QTY", addParm.getDouble("PLAN_QTY", i));  
                table.setItem(row, "PLAN_QTY", addParm.getDouble("PLAN_QTY", i));
                table.setItem(row, "PUR_QTY", addParm.getDouble("PLAN_QTY", i));
                table.setItem(row, "CHECK_QTY", addParm.getDouble("PLAN_QTY", i)); 
    ...
    private void check_qty_popedom() {
        ...
        if (plan_qty_popedom && pur_qty_popedom && check_qty_popedom) {
            getTable("TABLE_D").setLockColumns(
                "0,1,2,3,4,5,6,7,11,12,13,14,15,16,17,18,19");
        }
        // 调整生成量权限,调整修改量权限
