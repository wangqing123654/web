...
/**
 * <p>Title: 科室发药成本统计报表(以科室为单位)</p>
 *
 * <p>Description: 科室发药成本统计报表(以科室为单位)</p>
 *
 ...
 * @version 1.0
 */
public class INDDispenseCostQueryDeptControl
    extends TControl {

    public TTable table_1;  

    public INDDispenseCostQueryDeptControl() {   
    }
    ...
    public void onQuery() {
        ...
        if (!"".equals(this.getValueString("ORG_CODE"))) {
            ...
            where_2 += " AND A.EXE_DEPT_CODE = '" + getValueString("ORG_CODE") +
                "' ";
            where_3 += " AND M.TO_ORG_CODE = '" + getValueString("ORG_CODE") +
            "' ";
        }    

         //fux modify 20151020 包含补充计费   将补充计费 并入 发药成本统计中  
        String sql_1 = " SELECT   B.DEPT_CHN_DESC, SUM (A.AR_AMT) "
            +
            //" AS OWN_AMT, SUM (A.COST_AMT) AS COST_AMT, D.PHA_TYPE, A.DEPT_CODE "
            " AS OWN_AMT, SUM (A.COST_AMT) AS COST_AMT, D.TYPE_CODE, A.DEPT_CODE "
            + " FROM OPD_ORDER A, SYS_DEPT B, PHA_BASE D "
            + " WHERE A.DEPT_CODE = B.DEPT_CODE "
            + " AND A.ORDER_CODE = D.ORDER_CODE "      
            + " AND A.PHA_DOSAGE_DATE IS NOT NULL "  
            + " AND A.PHA_RETN_CODE IS NULL "
            + " AND B.FINAL_FLG = 'Y' " 
            //因为 妇产科手术室 儿科手术室 这些 CLASSIFY 分类 为 1   
            + " AND B.CLASSIFY = '0' "
            ...
            + " GROUP BY B.DEPT_CHN_DESC, D.TYPE_CODE, "      
            + " A.DEPT_CODE  " + 
              " ORDER BY A.DEPT_CODE ";
        //System.out.println("sql_1----" + sql_1);   

        String sql_2 = "SELECT * FROM(  SELECT D.DEPT_CHN_DESC, SUM (A.OWN_AMT) "
            +
            //" AS OWN_AMT, SUM (A.COST_AMT) AS COST_AMT, C.PHA_TYPE, A.DEPT_CODE "
            " AS OWN_AMT, SUM (A.COST_AMT) AS COST_AMT, C.TYPE_CODE, A.DEPT_CODE "
            ...
            + " GROUP BY D.DEPT_CHN_DESC, C.TYPE_CODE, "                      
            + " A.DEPT_CODE " +
            "  UNION (SELECT   A.DEPT_CHN_DESC, SUM (D.QTY*D.RETAIL_PRICE) "+
...
//                "', 'YYYYMMDDHH24MISS') " + where + where_1 +                     
    ...
    public void onPrint() {
        ...
        this.openPrintWindow(
            "%ROOT%\\config\\prt\\spc\\INDDispenseCostQueryDept.jhw", date);
