...
//		}
		...
		String clearString = "START_DATE;END_DATE;REQUEST_DATE;REQUEST_NO;" + "REQTYPE_CODE;TO_ORG_CODE;REASON_CHN_DESC;DESCRIPTION;"
...
//		((TMenuItem) getComponent("import")).setEnabled(false);
		...
		if("N".equals(is_separate_req)){
			// modify by wangbin 2014/11/27 如果没有麻精，则禁用分类，默认普药
			((TLabel)getComponent("tLabel_6")).setEnabled(false);
			((TRadioButton)getComponent("GEN_DRUG")).setEnabled(false);
			((TRadioButton)getComponent("TOXIC_DRUG")).setEnabled(false);
			((TRadioButton)getComponent("NOT_DRUG")).setEnabled(false);
			
			((TRadioButton)getComponent("GEN_DRUG")).setSelected(true);
