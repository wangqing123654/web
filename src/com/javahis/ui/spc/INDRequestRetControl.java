...
//                         StringTool.round(Double.parseDouble(this.
    ...
    public void popReturn(String tag, Object obj) {
        ...
        //fux modify  20141112 药品种类 -- 是否分单    
		TParm resultFd = new TParm(TJDODBTool.getInstance().select(INDSQL.getINDSysParm()));
		String is_separate_req = resultFd.getValue("IS_SEPARATE_REQ",0);    
        if(is_separate_req.equals("Y")){
        	 TParm isDrugParm = new TParm(TJDODBTool.getInstance().select(INDSQL.isDrug(order_code)));
             if(getRadioButton("G_DRUGS").isSelected()&&getRadioButton("G_DRUGS").isVisible()){//非麻精
             ...
             }else if(getRadioButton("N_DRUGS").isSelected()&&getRadioButton("N_DRUGS").isVisible()){//麻精
...
/*            TParm inparm = new TParm(TJDODBTool.getInstance().select(INDSQL.
            ...
            if (qty > stock_qty) {
                TParm order = INDTool.getInstance().getSysFeeOrder(   
                    order_code);
              //fux modify 20150422 改为强制不可保存
              this.messageBox("药品:" + order.getValue("ORDER_DESC") + "(" +
              order_code + ") 库存数量不足,当前库存量为" + stock_qty+",不可申请退库！");  
