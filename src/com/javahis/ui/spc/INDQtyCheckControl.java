...
public class INDQtyCheckControl
    ...
    public void onLock() {
        ...
        boolean flgOnway = false;  
        if ("0".equals(checkreason)) {
        	 for (int i = 0; i < parm.getCount("ORDER_CODE"); i++) {
             	//String orderCode = parm.getValue("ORDER_CODE",i).toString();    
//             	String sql = " SELECT ORDER_CODE FROM IND_STOCKM WHERE ORG_CODE = '"+org_code+"'  AND ORDER_CODE = '"+orderCode+"' " +
//             			" AND BUY_UNRECEIVE_QTY !=0 ";  
             	String sql = " SELECT D.ORDER_CODE FROM IND_DISPENSEM M,IND_DISPENSED D WHERE M.DISPENSE_NO = D.DISPENSE_NO " +
             			" AND M.APP_ORG_CODE = '"+org_code+"'  " +
             			//" AND D.ORDER_CODE = '"+orderCode+"' " +   
             	//在途          
     			" AND M.UPDATE_FLG = '1' ";    
             	//System.out.println("sql:"+sql);  
             	TParm parmflgOnway = new TParm(TJDODBTool.getInstance().select(sql));
                 if (parmflgOnway.getCount("ORDER_CODE")>0) {
                 	flgOnway = true;
                     break;         
                 }  
             }
             if (flgOnway) {
                 this.messageBox("有在途库存");  
             }
        }
        ...
        //fux modify 20141027 IND_STOCK STOCK_FLG为冻结标记
        //IND_STOCK &
