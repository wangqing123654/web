...
//                    System.out.println("dept_code"+dept_code); 
    ...
    public void onSave() {
        ...
        //fux modify 20150907  请在退库出库界面完成出库操作
    	if ("RET".equals(request_type)) {
    		messageBox("请在退库出库界面完成出库操作!");
    		return;
    	}  
        ...
        if (getRadioButton("UPDATE_FLG_B").isSelected()) {
            ...
            if ("1".equals(dis_check)) {
                ...
                //fux modify 20160111 科室退 不校验 库存
...
//                    this.messageBox(message);
    ...
    private boolean queryExistRequestNo(){
    	...
    	String sql = "SELECT REQUEST_DATE FROM IND_REQUESTM WHERE REQUEST_NO = '"+request_no+"'";
...
//            parm.setData("ORG_CODE", out_org_code);//20150527 wangjc modify
            parm.setData("ORG_CODE", this.getValueString("TO_ORG_CODE"));//20150527 wangjc add
...
//            System.out.println("parm22222:"+parm);  
    ...
    public void onPrint() {
        ...
        if (table_d.getRowCount() > 0) {
            ...
            date.setData("selectBy",this.getValueString("TO_ORG_CODE"));
            date.setData("ORG_CODE_FROM", "TEXT", "接受部门: " +
                         this.getComboBox("TO_ORG_CODE").getSelectedName());
            date.setData("REASON_CHN_DESC", "TEXT", "申请原因: " +
                    this.getComboBox("REASON_CHN_DESC").getSelectedName());
            ...
            String sql =
                    ...
                    + "  END AS ORDER_DESC, "
                    + " B.SPECIFICATION, C.UNIT_CHN_DESC,A.UNIT_CODE, "
...
//    		this.getValueString("TO_ORG_CODE")+"' AND ORDER_CODE='";
			...
			for (int i = 0; i < printData.getCount("ORDER_CODE"); i++) {
				//System.out.println("::::::::::::"+newsql+printData.getData("ORDER_CODE", i)+"'");
...
//				parm.addData("MATERIAL_LOC_CODE", newParm.getValue("MATERIAL_LOC_CODE",0));//料位号 add by huangjw 20150415
				...
				parm.addData("QTY", StringTool.round(qty,2));
				parm.addData("AMT", StringTool.round(printData.getDouble(
						"RETAIL_PRICE", i)
						* qty, 2));
				parm.addData("AMT_VERIFYIN", df2.format((printData.getDouble(
						"VERIFYIN_PRICE", i) * qty)));
				// parm.addData("AMT_RETAIL", df2.format((printData.getDouble(
				// "RETAIL_PRICE", i) * qty)));
				...
				// 20150507 wangjingchun add start
...
//				System.out.println(stockParm);
				...
				//查供货厂商 modify by wangb 2016/3/28 供货商取值修改 START
				String supSql = "SELECT SUP_ABS_DESC FROM IND_STOCK A,SYS_SUPPLIER B WHERE "
						+ " A.SUP_CODE = B.SUP_CODE AND ORG_CODE='"
						+ this.getValueString("TO_ORG_CODE")
						+ "' AND BATCH_NO = '"
						+ printData.getValue("BATCH_NO", i)
						+ "' AND ORDER_CODE = '"
				...
				//查供货厂商 modify by wangb 2016/3/28 供货商取值修改 END
				
				supParm = new TParm(TJDODBTool.getInstance().select(supSql));
            ...
            parm.addData("SYSTEM", "COLUMNS", "MATERIAL_LOC_CODE");//料位
            parm.addData("SYSTEM", "COLUMNS", "ORDER_DESC");//药品名称
            parm.addData("SYSTEM", "COLUMNS", "SPECIFICATION");//规格 
            parm.addData("SYSTEM", "COLUMNS", "UNIT");//单位
            parm.addData("SYSTEM", "COLUMNS", "QTY");//数量                      
            parm.addData("SYSTEM", "COLUMNS", "VERIFYIN_PRICE");//采购单价
            parm.addData("SYSTEM", "COLUMNS", "AMT_VERIFYIN");//采购金额
            parm.addData("SYSTEM", "COLUMNS", "UNIT_PRICE");//零售金额
            //parm.addData("SYSTEM", "COLUMNS", "AMT_RETAIL");
            parm.addData("SYSTEM", "COLUMNS", "BATCH_NO");//批号//20150210 wangjingchun add
            parm.addData("SYSTEM", "COLUMNS", "VALID_DATE");//效期//20150210 wangjingchun add
            parm.addData("SYSTEM", "COLUMNS", "STOCK_QTY");//结存量//20150327 wangjingchun add
            parm.addData("SYSTEM", "COLUMNS", "SUP_CODE");//供应商
            date.setData("TABLE", parm.getData());
            //System.out.println("打印Parm：：：：：：：：："+date);
            // 表尾数据
            //luhai 2012-1-22 加入合计atm
            String atm = df2.format(sum_retail_price)+"";
            String verifyinAtm=df2.format(sum_verifyin_price)+"";  
...
//            date.setData("ATM", "TEXT",atm);//20150507 wangjingchun 去掉 
            ...
            date.setData("BAR_CODE", "TEXT", this.getValueString("DISPENSE_NO"));
            // add by wangbin 20140929 需求单号937 START
            //date.setData("ATTN", "TEXT", "经办人: " + Operator.getName());
//            date.setData("ATTN", "TEXT",Operator.getName()); //20150507 wangjingchun 去掉 
            // add by wangbin 20140929 需求单号937 END   
			// 调用打印方法
    ...
    public void onTableMClicked() {
        ...
        if (row_m != -1) {
            ...
            setValue("SUM_RETAIL_PRICE", Math.round(getSumRetailMoney()));//20150609 wangjc modify 四舍五入
    ...
    public boolean onTableDChangeValue(Object obj) {
        ...
        if (column == 8) {
            ...
            //modify by yangjj 20150316 去掉等于0限制
            if (price < 0) {
                this.messageBox("成本价不能小于0");
                return true;
            }
    ...
    private TParm onQueryParm() {
        ...
        }else if (getRadioButton("A_DRUGS").isSelected()) {
//        	parm.setData("DRUG_CATEGORY","3");//20151016 wangjc add 选择全部时
        }    
...
//            || "COS".equals(request_type)) {
        //fux modify 20150408 
        if ("TEC".equals(request_type) || "EXM".equals(request_type)
                || "COS".equals(request_type) || "GIF".equals(request_type)) {
//            u_type = "1";
        	u_type = "0";//20150728 wangjc modify 全部使用库存单位
        }
        ...
        //fux modify GIF   
        if ("DEP".equals(request_type) || "TEC".equals(request_type)||"GIF".equals(request_type)) {
...
/*    	TParm result = new TParm(TJDODBTool.getInstance().select(sqlGetIndDispenseDByNo));
		...
		sqlGetIndDispenseDByNo =   
			...
			" END AS ORDER_DESC," +                               
			" B.SPECIFICATION, A.QTY, A.ACTUAL_QTY, A.UNIT_CODE," +    
//			" A.VERIFYIN_PRICE AS  STOCK_PRICE, " +   //20150609 wangjc modify
			" CASE WHEN A.UNIT_CODE=G.DOSAGE_UNIT "+//20150609 wangjc add
			" THEN A.VERIFYIN_PRICE ELSE A.VERIFYIN_PRICE*G.DOSAGE_QTY END AS STOCK_PRICE,"+//20150609 wangjc add
			" CASE WHEN A.UNIT_CODE=G.DOSAGE_UNIT "+//20150609 wangjc add
			" THEN A.RETAIL_PRICE ELSE A.RETAIL_PRICE*G.DOSAGE_QTY END AS RETAIL_PRICE,"+//20150609 wangjc add
//            " A.RETAIL_PRICE   AS RETAIL_PRICE," +//20150609 wangjc modify
...
//            " B.RETAIL_PRICE   AS RETAIL_PRICE," +
			...
			",PHA_TRANSUNIT G  " +//20150609 wangjc modify
			" WHERE A.ORDER_CODE = B.ORDER_CODE" +  
			" AND A.ORDER_CODE = C.ORDER_CODE" +  
            ...
            " AND A.REQUEST_SEQ = F.SEQ_NO" +  
			" AND A.DISPENSE_NO = '" + dispense_no + "' "
			+ " AND A.ORDER_CODE=G.ORDER_CODE ";//20150609 wangjc add
...
//            u_type = "1";
    ...
    private TParm setTableDValue(TParm result) {
        ...
        for (int i = 0; i < result.getCount(); i++) {
            ...
            unit_code = result.getValue("UNIT_CODE", i);
            ...
            if (!"".equals(result.getValue("BATCH_NO", i))) {
                ...
                if ("0".equals(u_type)) {
                	if(un==3){
                    parm.setData("STOCK_QTY", i,
                    		//20150706 wangjc modify 库存数量只显示在校期内的和ACTIVE_FLG='Y'的
//                    		this.getStockQTY(out_org_code, order_code)
                                 INDTool.getInstance().getStockQTY(
                                     out_org_code, order_code));
                	}else{
                        parm.setData("STOCK_QTY", i,
...
//                    * result.getDouble("DOSAGE_QTY", i);
            	//fux modify 20150424 
            	stock_price = result.getDouble("STOCK_PRICE", i);    
                retail_price = result.getDouble("RETAIL_PRICE", i);//20150609 wangjc modify
//                    * result.getDouble("DOSAGE_QTY", i);  
            }
            else {
                // 配药单位
	...
	public double getStockQTY(String org_code, String order_code) {
		...
		String sql1 = "SELECT SUM(STOCK_QTY) AS QTY "
				+ " FROM IND_STOCK "
				+ " WHERE ORG_CODE = '" + org_code
				+ "' AND ORDER_CODE = '" + order_code + "' AND ACTIVE_FLG='Y' AND SYSDATE < VALID_DATE ";
		...
		TParm resultBase = IndStockDTool.getInstance().onQueryStockBaseQTY(parm);
		if (resultBase == null || resultBase.getErrCode() < 0) {
			err("ERR:" + resultBase.getErrCode() + resultBase.getErrText() + resultBase.getErrName());
			return -1;
		}
    ...
    private boolean CheckDataD() {
        ...
        for (int i = 0; i < table_d.getRowCount(); i++) {
            ...
            //modify by yangjj 20150316 去掉不能等于0限制
            if (price < 0) {
                this.messageBox("成本价不能小于0");
                return false;
            }
...
//        }
        ...
        //fux modify 20160108
        parm.setData("APP_ORG_CODE", this.getValueString("APP_ORG_CODE"));//20150727 wangjc add
        parm.setData("TO_ORG_CODE", this.getValueString("TO_ORG_CODE"));//20150727 wangjc add
        // 执行数据新增  
    ...
    private TParm getDispenseMParm(TParm parm, String update_flg) {
        ...
        if ("WAS".equals(getValue("REQTYPE_CODE")) ||
            ...
            "DRT".equals(getValue("REQTYPE_CODE"))) {
            update_flg = "3";
        }
    ...
    private TParm getDispenseDParm(TParm parm) {
        ...
        TParm dateParm = table_d.getParmValue();
//        System.out.println("resultParm>>>>>"+resultParm);
        TParm newParm = new TParm();
        int k=0;
        for(int n=0;n<dateParm.getCount("ORDER_CODE");n++){
//        	if(!dateParm.getBoolean("SELECT_FLG", n)){
//        		continue;
//        	}
        	if(newParm.getCount("ORDER_CODE")<=0){
        		newParm.setData("SELECT_FLG", k, dateParm.getValue("SELECT_FLG", n));
        		newParm.setData("ORDER_CODE", k, dateParm.getValue("ORDER_CODE", n));
        		newParm.setData("QTY", k, dateParm.getDouble("QTY", n));
        		newParm.setData("QTY_1", k, dateParm.getDouble("QTY", n));
        		newParm.setData("UNIT_CODE", k, dateParm.getValue("UNIT_CODE", n));
        		newParm.setData("RETAIL_PRICE", k, dateParm.getDouble("RETAIL_PRICE", n));
        		newParm.setData("STOCK_PRICE", k, dateParm.getDouble("STOCK_PRICE", n));
        		newParm.setData("ACTUAL_QTY", k, dateParm.getDouble("OUT_QTY", n));
        		newParm.setData("ACTUAL_QTY_1", k, dateParm.getDouble("OUT_QTY", n));
        		newParm.setData("PHA_TYPE", k, dateParm.getValue("PHA_TYPE", n));
        		newParm.setData("BATCH_SEQ", k, dateParm.getInt("BATCH_SEQ", n));
        		newParm.setData("BATCH_NO", k, dateParm.getValue("BATCH_NO", n));
        		newParm.setData("VALID_DATE", k, dateParm.getValue("VALID_DATE", n));
        		k++;
        	}else{
        		boolean flg=true;
        		for(int j=0;j<newParm.getCount("ORDER_CODE");j++){
        			if(newParm.getValue("ORDER_CODE", j).equals(dateParm.getValue("ORDER_CODE", n))){
        				if(newParm.getBoolean("SELECT_FLG", j) && dateParm.getBoolean("SELECT_FLG", n)){
        					newParm.setData("QTY", j, dateParm.getDouble("QTY", n)+newParm.getDouble("QTY", j));
        					newParm.setData("ACTUAL_QTY", j, dateParm.getDouble("OUT_QTY", n)+newParm.getDouble("ACTUAL_QTY", j));
        					
        					newParm.setData("SELECT_FLG", k, dateParm.getValue("SELECT_FLG", n));
                			newParm.setData("ORDER_CODE", k, dateParm.getValue("ORDER_CODE", n));
                			newParm.setData("QTY", k, 0);
                			newParm.setData("QTY_1", k, dateParm.getDouble("QTY", n));
                			newParm.setData("UNIT_CODE", k, dateParm.getValue("UNIT_CODE", n));
                			newParm.setData("RETAIL_PRICE", k, dateParm.getDouble("RETAIL_PRICE", n));
                			newParm.setData("STOCK_PRICE", k, dateParm.getDouble("STOCK_PRICE", n));
                			newParm.setData("ACTUAL_QTY", k, 0);
                			newParm.setData("ACTUAL_QTY_1", k, dateParm.getDouble("OUT_QTY", n));
                			newParm.setData("PHA_TYPE", k, dateParm.getValue("PHA_TYPE", n));
                			newParm.setData("BATCH_SEQ", k, dateParm.getInt("BATCH_SEQ", n));
                			newParm.setData("BATCH_NO", k, dateParm.getValue("BATCH_NO", n));
        					...
        					flg=false;
        					break;
        				}else if(j == newParm.getCount("ORDER_CODE")-1){
        					newParm.setData("SELECT_FLG", k, dateParm.getValue("SELECT_FLG", n));
                			newParm.setData("ORDER_CODE", k, dateParm.getValue("ORDER_CODE", n));
                			newParm.setData("QTY", k, dateParm.getDouble("QTY", n));
                			newParm.setData("QTY_1", k, dateParm.getDouble("QTY", n));
...
//                continue;
//            }
        	if(!newParm.getBoolean("SELECT_FLG", i)){
        		continue;
        	}
...
//                "PHA_TYPE"));
          //20150720 wangjc modify end
            //20150720 wangjc add start
            parmD.setData("QTY", count, newParm.getDouble("QTY", i));
            parmD.setData("QTY_1", count, newParm.getDouble("QTY_1", i));
            parmD.setData("UNIT_CODE", count, newParm.getValue("UNIT_CODE", i));
            parmD.setData("RETAIL_PRICE", count, newParm.getDouble("RETAIL_PRICE", i));
            parmD.setData("STOCK_PRICE", count, newParm.getDouble("STOCK_PRICE", i));
            parmD.setData("ACTUAL_QTY", count, newParm.getDouble("ACTUAL_QTY", i));
            parmD.setData("ACTUAL_QTY_1", count, newParm.getDouble("ACTUAL_QTY_1", i));
            parmD.setData("PHA_TYPE", count, newParm.getValue("PHA_TYPE", i));
...
//                "BATCH_SEQ"));
//              parmD.setData("BATCH_SEQ",count,table_d.getParmValue().getInt("BATCH_SEQ",i));//20150720 wangjc modify
              parmD.setData("BATCH_SEQ",count,newParm.getInt("BATCH_SEQ", i));//20150720 wangjc add
            //luahi modify 2012-1-16 batch_seq 的值需要从Tparm中取得，因为页面中并不体现batch_seq end
            
//            batch_no = table_d.getItemString(i, "BATCH_NO");//20150720 wangjc modify
            batch_no = newParm.getValue("BATCH_NO", i);//20150720 wangjc add
            parmD.setData("BATCH_NO", count, batch_no);
//            valid_date = table_d.getItemString(i, "VALID_DATE");//20150720 wangjc modify
            valid_date = newParm.getValue("VALID_DATE", i);//20150720 wangjc add
            if ("".equals(valid_date)) {
                parmD.setData("VALID_DATE", count, tnull);
            }
            else {
//					parmD.setData("VALID_DATE", count,
//                              table_d.getItemTimestamp(i, "VALID_DATE"));//20150720 wangjc modify
            	parmD.setData("VALID_DATE", count,
					Timestamp.valueOf(valid_date.substring(0, 10).replace("/", "-")+" 00:00:00"));//20150720 wangjc add
            }
...
//        return message;
    	...
    	for (int i = 0; i < dParm.getCount("ORDER_CODE"); i++) {
    		if (!dParm.getBoolean("SELECT_FLG", i)) {
              continue;
    		}
    		if(parm.getCount()<=0){
    			parm.addData("ORDER_CODE", dParm.getValue("ORDER_CODE", i));
    			parm.addData("ORDER_DESC", dParm.getValue("ORDER_DESC", i));
    			parm.addData("STOCK_QTY", dParm.getValue("STOCK_QTY", i));
    			parm.addData("OUT_QTY", dParm.getValue("OUT_QTY", i));
    			parm.setCount(n);
    			n++;
    		}else{
    			boolean flg = true;
    			for(int j=0;j<parm.getCount();j++){
    				if(parm.getValue("ORDER_CODE", j).equals(dParm.getValue("ORDER_CODE", i))){
    					parm.setData("OUT_QTY", j, parm.getDouble("OUT_QTY", j)+dParm.getDouble("OUT_QTY", i));
    					flg=false;
    					break;
    				}
    			}
    	...
    	for(int k=0;k<parm.getCount();k++){
    		if(parm.getDouble("OUT_QTY", k)>parm.getDouble("STOCK_QTY", k)){
    			message += parm.getValue("ORDER_DESC", k) + "\n";
    			for (int i = 0; i < dParm.getCount("ORDER_CODE"); i++) {
    				if(parm.getValue("ORDER_CODE", k).equals(dParm.getValue("ORDER_CODE", i))){
    					table_d.setItem(i, "SELECT_FLG", "N");
    				}
    			}
    		}
