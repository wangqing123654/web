 ...
 * @version 1.0
 */
public class INDRegressgoodsControl extends TControl {
	/**
	 * 操作标记
	 */
	...
	public void onClear() {
		...
		// 清空画面内容
		String clearString = "REGRESSGOODS_DATE;ORG_CODE;REGRESSGOODS_NO;"
	 ...
	 */
	public void onSave() {
		if (this.getRadioButton("UPDATE_FLG_B").isSelected()) {//20150528 wangjc add
			...
			Timestamp date = SystemTool.getInstance().getDate();
			// fux modify 20141027 注意table值是否同步 值改变事件需要修正
			...
			for (int i = 0; i < table.getRowCount(); i++) {
				...
				// System.out.println("qtyCheck * sQty:" + (qtyCheck * sQty));
				// System.out.println("qtySum * sQty:" + (qtySum * sQty));
				// System.out.println("qtyCheck * sQty + qtySum * sQty:"
				// + (qtyCheck * sQty + qtySum * sQty));
				// System.out.println("verQty*sQty:" + (verQty * sQty));
			...
			if ("insert".equals(action)) {
				...
				for (int i = 0; i < newrows.length; i++) {
					...
					// **********************************************************************************
					// luhai modify 2012-01-11 batchSeq 在验收时已经记录，删除根据批次效期查询batchSeq
					// begin
					// **********************************************************************************
					// // 出入库效期批号者
					// getSEQ = onQueryStockBatchSeq(org_code, order_code, batch_no,
					// valid_date);
					// if (getSEQ.getErrCode() < 0 || getSEQ.getCount("BATCH_SEQ")
					// <= 0) {
					...
					batch_seq = StringTool.getInt(dataStore.getItemString(
							newrows[i], "BATCH_SEQ"));
					parm.setData("BATCH_SEQ", count, batch_seq);
					// **********************************************************************************
					// luhai modify 2012-01-11 batchSeq 在验收时已经记录，删除根据批次效期查询batchSeq
					...
					//
					String supOrderCode = dataStore.getItemString(newrows[i],
							"SUP_ORDER_CODE");
				...
				if ("Y".equals(check_flg)) {
					...
					for (int i = 0; i < table_D.getRowCount(); i++) {
						...
						// String batchSeq = dataStore.getItemString(i,
						// "BATCH_SEQ");
						...
						// }
						// batch_seq = getSEQ.getInt("BATCH_SEQ", 0);
						// String batchSeqStr = dataStore.getItemString(i,
						// "BATCH_SEQ");
						// System.out.println("======"+batchSeqStr);
						batch_seq = StringTool.getInt(dataStore.getItemString(i,
								"BATCH_SEQ"));
						// luhai modify 2012-05-07 begin 扣库根据batchSeq 进行处理 begin
						// 出入库效期批号者集合
						...
						inparm.setData("VERIFYIN_NO", table_D.getItemString(i,
								"VERIFYIN_NO"));
						inparm
						.setData("SEQ_NO", table_D.getItemInt(i,
								"VERSEQ_NO"));
				...
				if ("Y".equals(check_flg)) {
					// 根据退货单号判断退货出库量更新入库状态
					updateIndRegressGoodsDUpdateFlg(getValueString("REGRESSGOODS_NO"));
				}
			...
			if (getRadioButton("UPDATE_FLG_B").isSelected()) {
				...
				for (int i = 0; i < table_D.getRowCount(); i++) {
					...
					if (table_D.getDataStore().isActive(i)) {
						parmD.addData("REGRESSGOODS_NO", this
								.getValueString("REGRESSGOODS_NO"));
						parmD.addData("ORDER_CODE", table_D.getDataStore()
								.getRowParm(i).getValue("ORDER_CODE"));
						parmD.addData("BATCH_NO", table_D.getDataStore()
								.getRowParm(i).getValue("BATCH_NO"));
						parmD.addData("VALID_DATE", table_D.getDataStore()
								.getRowParm(i).getValue("VALID_DATE").substring(0,
										10).replace("-", "/"));
						parmD.addData("INVOICE_NO", table_D.getDataStore()
								.getRowParm(i).getValue("INVOICE_NO"));
						parmD.addData("INVOICE_DATE", table_D.getDataStore()
								.getRowParm(i).getValue("INVOICE_DATE").substring(
										0, 10).replace("-", "/"));
						parmD.addData("OPT_USER", Operator.getID());
						parmD.addData("OPT_TERM", Operator.getIP());
						parmD.setCount(n);
						n++;
					}
				}
				...
				// 执行数据更新
				resultD = TIOM_AppServer.executeAction(
						"action.spc.INDVerifyinAction", "onUpdateRegoodsInvoiceNo",
						parmD);
				// 保存判断
				if (resultD == null || resultD.getErrCode() < 0) {
					this.messageBox("E0001");
					return;
				}
				this.messageBox("P0001");
			...
			if (parmD == null || parmD.getCount() <= 0) {
				this.messageBox("没有要更改的内容！");
				return;
			}
			// 执行数据更新
	...
	public void onExport() {
		...
		if (result != null) {
			...
			for (int i = 0; i < addParm.getCount("ORDER_CODE"); i++) {
				...
				double stockQty = INDTool.getInstance().getStockQTY(orgCode,
						orderCode);
				if (stockQty <= 0) {
				...
				// 单位
				table
						.setItem(row, "BILL_UNIT", addParm.getData("BILL_UNIT",
								i));
				// 退货单价
				verifyin_price = addParm.getDouble("VERIFYIN_PRICE", i);
				table.setItem(row, "UNIT_PRICE", verifyin_price);
				// 退货金额
				table.setItem(row, "AMT", StringTool.round(
						(verifyin_qty - actual_qty) * verifyin_price, 2));
				// 零售价
				...
				table.setItem(row, "BATCH_SEQ", addParm
						.getValue("BATCH_SEQ", i));
				// luhai 2012-01-11 add batchSeq end
				// luhai 2012-01-11 add verifyInPrice begin
				table.setItem(row, "VERIFYIN_PRICE", addParm.getValue(
						"VERIFYIN_PRICE", i));
				...
				table.setItem(row, "INVOICE_NO", addParm.getValue("INVOICE_NO",
						i));
	...
	public void onPrint() {
		...
		if (table_d.getRowCount() > 0) {
			...
			date.setData("REG_NO", "TEXT", "出库单号: "
					+ this.getValueString("REGRESSGOODS_NO"));
			date.setData("INVOICE_NO", "TEXT", "发票号: "
					+ table_d.getItemString(0, "INVOICE_NO"));
			date.setData("INVOICE_DATE", "TEXT", "发票日期: "
					+ table_d.getItemString(0, "INVOICE_DATE").substring(0, 10)
							.replace('-', '/'));
			date.setData("INVOICE_AMT", "TEXT", "发票金额: "
					+ this.getValueString("SUM_REG_ATM"));
			...
			for (int i = 0; i < table_d.getDataStore().rowCount(); i++) {
				...
				parm
						.addData("QTY", df3.format(table_d.getItemDouble(i,
								"QTY")));
				parm.addData("UNIT_PRICE", df4.format(table_d.getItemDouble(i,
						"UNIT_PRICE")));
				parm.addData("AMT", df2.format(StringTool.round(table_d
						.getItemDouble(i, "UNIT_PRICE")
						* table_d.getItemDouble(i, "QTY"), 2)));

				parm.addData("RETAIL_PRICE", df4.format(table_d.getItemDouble(
						i, "RETAIL_PRICE")));
				parm.addData("SELL_SUM", df2.format(StringTool.round(table_d
						.getItemDouble(i, "RETAIL_PRICE")
						* table_d.getItemDouble(i, "QTY"), 2)));
				// luhai 2012-2-11 add 零售价总额
				ownAmt += Double.parseDouble(df2.format(StringTool.round(
						table_d.getItemDouble(i, "RETAIL_PRICE")
								* table_d.getItemDouble(i, "QTY"), 2)));
				parm.addData("DIFF_SUM", df2.format((StringTool.round(table_d
						.getItemDouble(i, "RETAIL_PRICE")
						* table_d.getItemDouble(i, "QTY"), 2))
				...
				parm.addData("SUM_QTY", table_d.getItemDouble(i, "SUM_QTY"));
				...
				// 药品分类
				parm.addData("PHA_TYPE", phaTypeName);
				// 生产厂商
				parm
						.addData("MAN_CHN_DESC", inparm.getValue(
								"MAN_CHN_DESC", 0));
			}
			...
			date.setData("TOT", "TEXT", ""
					+ df2.format(StringTool.round(Double.parseDouble(this
							.getValueString("SUM_REG_ATM")), 2)));
			// luhai 2012-2-10 加入零售价总额
			date.setData("OWN_AMT", "TEXT", ""
					+ df2.format(StringTool.round(ownAmt, 2)));
			String format = df2.format(StringTool.round(Double.parseDouble(this
					.getValueString("SUM_REG_ATM")), 2));
			date.setData("TOT_DESC", "TEXT", StringUtil.getInstance()
					.numberToWord(Double.parseDouble(format)));
			date.setData("USER", "TEXT", "制表人: " + Operator.getName());
			// 调用打印方法
			TParm empty = new TParm();// 用于置空表格
			date.setData("TABLE11", empty.getData());
			this.openPrintWindow("%ROOT%\\config\\prt\\spc\\RegressGoods.jhw",
					date);

			// this.openPrintWindow("%ROOT%\\config\\prt\\spc\\RegressGoods2.jhw",
			// date);
		} else {
			this.messageBox("没有打印数据");
			return;
	...
	public void onTableMClicked() {
		...
		if (row != -1) {
			...
			if (tds.rowCount() == 0) {
				...
				// 查询该验收单的验收细项信息
				TParm parm = new TParm();
				parm.setData("VERIFYIN_NO", table_D.getItemString(0,
						"VERIFYIN_NO"));
				parm.setData("ORG_CODE", this.getValueString("ORG_CODE"));
				parm.setData("SUP_CODE", this.getValueString("SUP_CODE"));
				resultParm = IndVerifyinDTool.getInstance()
						.onQueryVerifyinDone(parm);
				getCheckBox("CHECK_FLG").setEnabled(true);
	...
	public void onTableDClicked() {
		...
		if (row != -1) {
			...
			// 取得SYS_FEE信息，放置在状态栏上
			String order_code = table.getDataStore().getItemString(
					table.getSelectedRow(), "ORDER_CODE");
	...
	public boolean onTableDChangeValue(Object obj) {
		...
		if ("QTY".equals(columnName)) {
			...
			String validDate = table.getItemString(row, "VALID_DATE")
					.substring(0, 10);
			// String batchSeq = table.getItemString(row, "BATCH_SEQ");
			...
			// fux modify 20141027
			// IND_VERIFYIND.ACTUAL_QTY
			// double actualQty = 0.00;
			// //以前已经退货数
			// double actAlQty = 0.00;
			// if(qty+actAlQty>actualQty){
			// this.messageBox("累计退货数量大于验收数不能退货");
			// }
			// 出入库数量转换率
			// TParm getTRA = INDTool.getInstance().getTransunitByCode(
			// orderCode);
			// double sQty = getTRA.getDouble("DOSAGE_QTY",0);
			// 这里把这段判断放到应该挪到保存时 -- 第一笔扣库100,库存变为0,第二笔要扣20时,系统就要弹出警告讯息
			...
			double amt = StringTool.round(qty
					* table.getItemDouble(row, "UNIT_PRICE"), 2);
		...
		if ("UNIT_PRICE".equals(columnName)) {
			...
			// modify by yangjj 20150316 去掉不能等于0的限制
			if (qty < 0) {
	...
	public void onChangeRadioButton() {
		if (this.getRadioButton("UPDATE_FLG_A").isSelected()) {
			((TMenuItem) getComponent("save")).setEnabled(true);//modify by wangjc 20171115 可以修改发票号和发票日期
			((TMenuItem) getComponent("export")).setEnabled(false);
		} else {
			((TMenuItem) getComponent("save")).setEnabled(true);
			((TMenuItem) getComponent("export")).setEnabled(true);
	...
	private void initPage() {
		/**
		 * 权限控制 权限1:一般个人权限,只显示自已所属科室 权限9:最大权限,显示全院药库部门
		 */
		// 显示全院药库部门
	...
	private String getRegressgoodsNO(String invoiceNo) {
		...
		if (!map.isEmpty() && map.containsKey(invoiceNo)) {
			...
			// System.out.println("a"+result[0] + "--b"+result[1] + ((String)
			// map.get(invoiceNo)));
		} else {
		...
		for (int i = 0; i < table.getRowCount(); i++) {
			if (table.getDataStore().isActive(i)) {
				...
				if ("insert".equals(action)) {
					...
					TParm inparm = new TParm();
					inparm.setData("VERIFYIN_NO", table.getItemString(i,
							"VERIFYIN_NO"));
				...
				// modify by yangjj 20150316
				if (qty < 0) {
					this.messageBox("退货单价不能小于0");
					return false;
				}
				// 发票号码
	...
	public void onInsertObserver() {
		...
		TDS tds = new TDS();
		tds.setSQL(INDSQL
				.getINDRegressgoodsD(getValueString("REGRESSGOODS_NO")));
		tds.retrieve();
		if (tds.rowCount() == 0) {
			seq = 1;
		} else {
	...
	private void updateIndRegressGoodsDUpdateFlg(String regressgoods_no) {
		String sql = "UPDATE IND_REGRESSGOODSD SET UPDATE_FLG = '3' WHERE REGRESSGOODS_NO = '"
				+ regressgoods_no + "'";
	 ...
	 * @return
	 */
	public String getIndStockBatchSeq(String org_code, String order_code,
	...
	public double getStockQTY(String org_code, String order_code) {
		...
		TParm result = new TParm(TJDODBTool.getInstance().select(
				getIndStockQty(org_code, order_code)));
		if (result == null || result.getErrCode() < 0) {
			err("ERR:" + result.getErrCode() + result.getErrText()
					+ result.getErrName());
			return -1;
