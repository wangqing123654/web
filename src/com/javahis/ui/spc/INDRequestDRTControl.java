 ...
 * </p>
 * 
 * 为了科室退 修改出库 的接受药房 还有扣库流程(加申请科室库存。。。)
 * 
 * <p>
 * Description: 科室退作业
 * </p>
 ...
 * @version 1.0
 */
public class INDRequestDRTControl extends TControl {
	...
	public void onPrint() {
		...
		if (table_d.getRowCount() > 0) {
			...
			date.setData("ORG_CODE_IN", "TEXT", "接受部门: "
			...
			for (int i = 0; i < table_d.getDataStore().rowCount(); i++) {
				...
				parm.addData("STOCK_AMT", StringTool.round(table_d
						.getItemDouble(i, "STOCK_PRICE")
	...
	public void onTableMClicked() {
		...
		if (row_m != -1) {
			...
			getComboBox("TO_ORG_CODE").setEnabled(false);
	...
	public void onChangeDept() {
		...
		getTextFormat("TO_ORG_CODE").setPopupMenuSQL(
				INDSQL.getIndOrgComobo(org_type, "", Operator.getRegion()));
		// getTextFormat("TO_ORG_CODE").setSQL();
	...
	public void popReturn(String tag, Object obj) {
		...
		// stock_price);
		// // 零售价
		// table_d.setItem(table_d.getSelectedRow(), "RETAIL_PRICE",
		// retail_price);
		// // 设定选中行的有效性
		...
		// 查询药品的批次和效期
		//fux modify 20171113 传参 0->1 改为小单位
		String sql = INDSQL.getOrderBatchNoValid(this
				.getValueString("TO_ORG_CODE"), order_code, "1");
		...
		if (addParm.getCount("ORDER_CODE") == 1) {
			...
			table_d.setItem(table_d.getSelectedRow(), "BATCH_NO", addParm
					.getValue("BATCH_NO", 0));
			// 有效期
			table_d.setItem(table_d.getSelectedRow(), "VALID_DATE", addParm
					.getTimestamp("VALID_DATE", 0));
			// ***********************************************************
			...
			// 加入verfiyin_price
			// double tranUnitQty = INDTool.getInstance().getPhaTransUnitQty(
			// order_code, "2");
			table_d.setItem(table_d.getSelectedRow(), "VERIFYIN_PRICE", addParm
					.getDouble("VERIFYIN_PRICE", 0));// *tranUnitQty
			// ***********************************************************
		...
		// 存在多批号和效期的数据
			...
			Object resultParm = openDialog(
					"%ROOT%\\config\\spc\\INDForVaild.x", qtyParm);
			...
			double tranUnitQty = INDTool.getInstance().getPhaTransUnitQty(
					order_code, "2");
			table_d.setItem(table_d.getSelectedRow(), "VERIFYIN_PRICE", addParm
					.getDouble("VERIFYIN_PRICE"));// *tranUnitQty luhai
			// 2012-05-03 删除转换率
			// ***********************************************************
			// luhai 2012-01-12 modify 加入batchSeq verifyinPrice end
			// ***********************************************************
		...
		// * result.getDouble("DOSAGE_QTY", 0), 2);
		stock_price = StringTool.round(result.getDouble("STOCK_PRICE", 0), 2);
		retail_price = StringTool.round(result.getDouble("RETAIL_PRICE", 0), 2);
		// 成本价
	...
	private void initPage() {
		...
		getComboBox("TO_ORG_CODE").setSQL(
				INDSQL.getIndOrgComobo("B", "", Operator.getRegion()));
		// getComboBox("TO_ORG_CODE").retrieve();
	...
	private boolean getOrgStockCheck() {
		...
		for (int i = 0; i < table_d.getDataStore().rowCount(); i++) {
			...
			// 校验科室退出库
			String sql = " SELECT A.ORDER_CODE,B.ORDER_DESC "
			...
			String sqlPha = " SELECT ORDER_DESC FROM PHA_BASE"
					+ " WHERE ORDER_CODE = '" + order_code + "' ";
			TParm PhaParm = new TParm(TJDODBTool.getInstance().select(sqlPha));
			if (checkParm.getCount("ORDER_CODE") <= 0) {
				this.messageBox("药品: "
						+ PhaParm.getValue("ORDER_DESC", 0).toString()
						+ ",非接受部门项目,请核实！");
				return false;
			}
