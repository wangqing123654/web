...
queryVerifyinDoneD.SQL=SELECT 'N' AS SELECT_FLG, B.VERIFYIN_NO, B.VERIFYIN_DATE, A.ORDER_CODE, C.SPECIFICATION, &
  		        ...
  		        FROM IND_VERIFYIND A, IND_VERIFYINM B, SYS_FEE C,(SELECT VERIFYIN_NO, VERSEQ_NO,SUM (QTY) AS QTY  FROM IND_REGRESSGOODSD GROUP BY VERIFYIN_NO, VERSEQ_NO) E , PHA_TRANSUNIT D &
 		        WHERE A.VERIFYIN_NO = B.VERIFYIN_NO &
			  AND A.VERIFYIN_NO = E.VERIFYIN_NO(+) &
			  AND A.SEQ_NO = E.VERSEQ_NO(+)       
   			  AND A.ORDER_CODE = C.ORDER_CODE &
   			  AND (A.UPDATE_FLG = '1' OR A.UPDATE_FLG = '3') & 
			  AND (A.ACTUAL_QTY > E.QTY OR E.QTY IS　NULL) &       
   			  AND A.ORDER_CODE = D.ORDER_CODE &   
                 	  AND C.ORDER_CODE = D.ORDER_CODE &      
   			  AND B.ORG_CODE=<ORG_CODE> &    
   			  AND B.SUP_CODE=<SUP_CODE>
...
getQueryVerifyinBuyMaster.SQL=SELECT D.ORDER_DESC AS ORDER_DESC,D.SPECIFICATION, E.UNIT_CHN_DESC, &                         
							  ...
							  FROM IND_VERIFYINM A, IND_VERIFYIND B, SYS_FEE D, SYS_UNIT E ,PHA_BASE G &
...
getQueryVerifyinBuyDetail.SQL=SELECT A.VERIFYIN_NO, C.SUP_CHN_DESC, A.CHECK_DATE AS VERIFYIN_DATE, &
							  ...
							  B.REASON_CHN_DESC,B.QUALITY_DEDUCT_AMT,B.PURORDER_NO,B.PURSEQ_NO,C.SUP_ABS_DESC,B.MAN_DATE,D.HYGIENE_TRADE_CODE &
...
getQueryVerifyinGiftMaster.SQL=SELECT  D.ORDER_DESC AS ORDER_DESC,D.SPECIFICATION, E.UNIT_CHN_DESC, &                         
							  ...
							  D.MAN_CODE AS MAN_NAME  &
							 FROM IND_VERIFYINM A, IND_VERIFYIND B, SYS_FEE D, SYS_UNIT E &
							 WHERE A.VERIFYIN_NO = B.VERIFYIN_NO &
							   AND B.ORDER_CODE = D.ORDER_CODE &
							   AND B.BILL_UNIT = E.UNIT_CODE &
							   AND B.UPDATE_FLG IN ('1', '3') &
