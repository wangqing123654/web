...
Module.item=queryStockQTY;updateStockQtyOut;updateStockQtyCancelOut;updateStockQtyVer;createNewStockD;&
	    ...
	    updateStockQtyDisInReq;updateStockQtyDisInThi;updateProfitLossAmt;getOrgStockQuery;getOrgStockQueryNotBatch;updateStockQtyDisInDRT;&
	    updateStockQtyDisInRet;getDrugMianStockQuery;onQueryLockQTY;updateIndStockLockQty;getOrgStockDrugQuery;onQueryBaseQTY
//根据药库编号及药品代码查询药库库存量
...
//更新库存量(入库作业--DRT 加回来的 写到ret_in_qty里 库存加回来)
updateStockQtyDisInDRT.Type=TSQL
updateStockQtyDisInDRT.SQL=UPDATE IND_STOCK &
			   SET IN_QTY=IN_QTY+<OUT_QTY>, &
			       IN_AMT =IN_AMT+<OUT_AMT>, &
			       STOCK_QTY=STOCK_QTY+<STOCK_QTY>, &
			       STOCKIN_QTY=STOCKIN_QTY+<STOCKOUT_QTY>, &
			       STOCKIN_AMT=STOCKIN_AMT+<STOCKOUT_QTY>, &
			       RET_IN_QTY=RET_IN_QTY+<STOCKOUT_QTY>, &
			       RET_IN_AMT=RET_IN_AMT+<STOCKOUT_QTY>, &
			       OPT_USER=<OPT_USER>, &
			       OPT_DATE=<OPT_DATE>, &
			       OPT_TERM=<OPT_TERM> &
		         WHERE ORG_CODE=<ORG_CODE> &
			   AND ORDER_CODE=<ORDER_CODE> &
			   AND BATCH_SEQ=<BATCH_SEQ>
updateStockQtyDisInDRT.Debug=N
...
//根据药库编号及药品代码查询科室备药存量  add by huangtt 20150608
onQueryBaseQTY.Type=TSQL
onQueryBaseQTY.SQL=SELECT SUM(QTY) QTY,B.UNIT_CODE FROM IND_BASEMANAGEM A,IND_BASEMANAGED B &
					WHERE A.BASEMANAGE_NO=B.BASEMANAGE_NO GROUP BY B.UNIT_CODE
onQueryBaseQTY.ITEM=ORG_CODE;ORDER_CODE
onQueryBaseQTY.ORG_CODE=A.TO_ORG_CODE=<ORG_CODE>
onQueryBaseQTY.ORDER_CODE=B.ORDER_CODE=<ORDER_CODE>
onQueryBaseQTY.Debug=N
